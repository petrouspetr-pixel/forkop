...
const CONFIG_NAME = getenv("TRAFIRA_CONFIG_NAME") || constant_value("TRAFIRA_CONFIG_NAME", "trafira");
const CONFIG_FILE = getenv("TRAFIRA_CONFIG_FILE") || "/etc/config/" + CONFIG_NAME;
const LIB_DIR = getenv("TRAFIRA_LIB") || "/usr/lib/trafira";
const BIN_PATH = getenv("TRAFIRA_BIN") || constant_value("TRAFIRA_BIN", "/usr/bin/trafira");
const SERVICE_INIT = getenv("TRAFIRA_SERVICE_INIT") || constant_value("TRAFIRA_SERVICE_INIT", "/etc/init.d/trafira");
const SERVICE_NAME = getenv("TRAFIRA_SERVICE_NAME") || constant_value("TRAFIRA_SERVICE_NAME", "trafira");
const LUCI_VIEW_DIR = getenv("TRAFIRA_LUCI_VIEW_DIR") || constant_value("TRAFIRA_LUCI_VIEW_DIR", "/www/luci-static/resources/view/trafira");
const LUCI_I18N_DOMAIN = getenv("TRAFIRA_LUCI_I18N_DOMAIN") || constant_value("TRAFIRA_LUCI_I18N_DOMAIN", "trafira");

const RUNTIME_STATE_DIR = getenv("TRAFIRA_RUNTIME_STATE_DIR") || "/var/run/trafira";
const SYSTEM_INFO_CACHE_FILE = getenv("TRAFIRA_SYSTEM_INFO_CACHE_FILE") || RUNTIME_STATE_DIR + "/system-info.json";
const RELOAD_STATE_FILE = getenv("TRAFIRA_RELOAD_STATE_FILE") || RUNTIME_STATE_DIR + "/reload-state";
const RELOAD_STATE_SNAPSHOT_FILE = getenv("TRAFIRA_RELOAD_STATE_SNAPSHOT_FILE") || RUNTIME_STATE_DIR + "/reload-state.snapshot." + clock()[0] + "." + clock()[1];
const PENDING_RELOAD_FILE = getenv("TRAFIRA_PENDING_RELOAD_FILE") || RUNTIME_STATE_DIR + "/reload.pending";
const SERVICE_TRIGGER_SYNC_FILE = getenv("TRAFIRA_SERVICE_TRIGGER_SYNC_FILE") || RUNTIME_STATE_DIR + "/service-triggers.sync";
const SUBSCRIPTION_UPDATE_STATE_DIR = getenv("TRAFIRA_SUBSCRIPTION_UPDATE_STATE_DIR") || RUNTIME_STATE_DIR + "/subscription-update";
const SUBSCRIPTION_LINKS_DIR = getenv("TRAFIRA_SUBSCRIPTION_LINKS_DIR") || RUNTIME_STATE_DIR + "/subscription-links";
const SUBSCRIPTION_METADATA_DIR = getenv("TRAFIRA_SUBSCRIPTION_METADATA_DIR") || RUNTIME_STATE_DIR + "/subscription-metadata";
const OUTBOUND_METADATA_DIR = getenv("TRAFIRA_OUTBOUND_METADATA_DIR") || RUNTIME_STATE_DIR + "/outbound-metadata";
const SECTION_CACHE_DIR = getenv("TRAFIRA_SECTION_CACHE_DIR") || RUNTIME_STATE_DIR + "/section-cache";
const RULE_CONDITION_CACHE_DIR = getenv("TRAFIRA_RULE_CONDITION_CACHE_DIR") || RUNTIME_STATE_DIR + "/rule-condition-cache";
const RUNTIME_CACHE_FORMAT_FILE = getenv("TRAFIRA_RUNTIME_CACHE_FORMAT_FILE") || RUNTIME_STATE_DIR + "/cache-format";
const PERSISTENT_SUBSCRIPTION_CACHE_DIR = getenv("TRAFIRA_PERSISTENT_SUBSCRIPTION_CACHE_DIR") || "/etc/trafira/subscription-cache";
const PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE = getenv("TRAFIRA_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE") || PERSISTENT_SUBSCRIPTION_CACHE_DIR + "/cache-format";
const PERSISTENT_SUBSCRIPTION_CACHE_FORMAT = getenv("TRAFIRA_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT") || "7";
const SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE = getenv("TRAFIRA_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE") || RUNTIME_STATE_DIR + "/subscription-bootstrap-retry.pid";
const DNS_FAILOVER_STATE_FILE = getenv("TRAFIRA_DNS_FAILOVER_STATE_FILE") || RUNTIME_STATE_DIR + "/dns-failover.json";
const DNS_FAILOVER_PID_FILE = getenv("TRAFIRA_DNS_FAILOVER_PID_FILE") || RUNTIME_STATE_DIR + "/dns-failover.pid";
...
const RELOAD_LOCK_DIR = getenv("TRAFIRA_RELOAD_LOCK_DIR") || "/var/run/trafira.reload.lock";
const INTERNAL_CONFIG_TRIGGER_GUARD = getenv("TRAFIRA_INTERNAL_CONFIG_TRIGGER_GUARD") || "/var/run/trafira.internal-config-change";

const LIST_UPDATE_CRON_MARKER = getenv("TRAFIRA_LIST_UPDATE_CRON_MARKER") || "# trafira-list-update";
const SUBSCRIPTION_UPDATE_CRON_MARKER = getenv("TRAFIRA_SUBSCRIPTION_UPDATE_CRON_MARKER") || "# trafira-subscription-update";
const COMPONENT_UPDATE_CHECK_CRON_MARKER = getenv("TRAFIRA_COMPONENT_UPDATE_CHECK_CRON_MARKER") || "# trafira-component-update-check";
const RELOAD_STATE_FORMAT = int(getenv("TRAFIRA_RELOAD_STATE_FORMAT") || "1");
const RUNTIME_CACHE_FORMAT = int(getenv("TRAFIRA_RUNTIME_CACHE_FORMAT") || "8");
const RUNTIME_STABLE_MIN_AGE = int(getenv("TRAFIRA_RUNTIME_STABLE_MIN_AGE") || "2");
const SING_BOX_START_STABLE_MIN_AGE = int(getenv("TRAFIRA_SING_BOX_START_STABLE_MIN_AGE") || "8");
const SING_BOX_START_VERIFY_TIMEOUT = int(getenv("TRAFIRA_SING_BOX_START_VERIFY_TIMEOUT") || "10");
const NFT_POPULATE_ENABLED_DEFAULT = int(getenv("TRAFIRA_NFT_POPULATE_ENABLED") || "1");
...
const RT_TABLE_NAME = constant_value("RT_TABLE_NAME", "trafira");
const NFT_TABLE_NAME = constant_value("NFT_TABLE_NAME", "TrafiraTable");
...
const NFT_COMMON_SET_NAME = constant_value("NFT_COMMON_SET_NAME", "trafira_subnets");
const NFT_COMMON6_SET_NAME = constant_value("NFT_COMMON6_SET_NAME", "trafira_subnets6");
const NFT_PORT_SET_NAME = constant_value("NFT_PORT_SET_NAME", "trafira_ports");
const NFT_IP_PORT_SET_NAME = constant_value("NFT_IP_PORT_SET_NAME", "trafira_ip_ports");
const NFT_IP_PORT6_SET_NAME = constant_value("NFT_IP_PORT6_SET_NAME", "trafira_ip6_ports");
const NFT_INTERFACE_SET_NAME = constant_value("NFT_INTERFACE_SET_NAME", "trafira_interfaces");
...
const SB_VARIANT_STATE_FILE = constant_value("SB_VARIANT_STATE_FILE", "/etc/trafira/sing-box-variant");
const SB_VERSION_STATE_FILE = constant_value("SB_VERSION_STATE_FILE", "/etc/trafira/sing-box-version");
...
let subscription_caches_prepared = getenv("TRAFIRA_SUBSCRIPTION_CACHES_PREPARED") || "0";
let subscription_runtime_no_refresh = getenv("TRAFIRA_SUBSCRIPTION_RUNTIME_NO_REFRESH") || "0";
...
function lifecycle_env() {
    return {
        TRAFIRA_CONFIG_NAME: CONFIG_NAME,
        TRAFIRA_LIB: LIB_DIR,
        TRAFIRA_BIN: BIN_PATH,
        TRAFIRA_SERVICE_INIT: SERVICE_INIT,
        TRAFIRA_RUNTIME_STATE_DIR: RUNTIME_STATE_DIR,
        TRAFIRA_SYSTEM_INFO_CACHE_FILE: SYSTEM_INFO_CACHE_FILE,
        TRAFIRA_SUBSCRIPTION_UPDATE_STATE_DIR: SUBSCRIPTION_UPDATE_STATE_DIR,
        TRAFIRA_SUBSCRIPTION_LINKS_DIR: SUBSCRIPTION_LINKS_DIR,
        TRAFIRA_SUBSCRIPTION_METADATA_DIR: SUBSCRIPTION_METADATA_DIR,
        TRAFIRA_OUTBOUND_METADATA_DIR: OUTBOUND_METADATA_DIR,
        TRAFIRA_SECTION_CACHE_DIR: SECTION_CACHE_DIR,
        TRAFIRA_RULE_CONDITION_CACHE_DIR: RULE_CONDITION_CACHE_DIR,
        TRAFIRA_RUNTIME_CACHE_FORMAT_FILE: RUNTIME_CACHE_FORMAT_FILE,
        TRAFIRA_RUNTIME_CACHE_FORMAT: as_string(RUNTIME_CACHE_FORMAT),
        TRAFIRA_PERSISTENT_SUBSCRIPTION_CACHE_DIR: PERSISTENT_SUBSCRIPTION_CACHE_DIR,
        TRAFIRA_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE: PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE,
        TRAFIRA_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT: PERSISTENT_SUBSCRIPTION_CACHE_FORMAT,
        TRAFIRA_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE: SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE,
        TRAFIRA_DNS_FAILOVER_STATE_FILE: DNS_FAILOVER_STATE_FILE,
        TRAFIRA_DNS_FAILOVER_PID_FILE: DNS_FAILOVER_PID_FILE,
        TRAFIRA_SUBSCRIPTION_UPDATE_LOCK_DIR: SUBSCRIPTION_UPDATE_LOCK_DIR,
        TRAFIRA_PENDING_RELOAD_FILE: PENDING_RELOAD_FILE,
        TRAFIRA_RELOAD_LOCK_DIR: RELOAD_LOCK_DIR,
        ...
        TRAFIRA_RULE_CONDITION_CACHE_ENABLED: as_string(rule_condition_cache_enabled)
    };
}
...
function start_main() {
    ...
    log_message("Starting Trafira", "info");
...
function start_impl() {
    ...
    else if (dnsmasq_has_trafira_managed_state()) {
...
function stop_main() {
    ...
    log_message("Stopping Trafira", "info");
...
function cleanup_failed_runtime() {
    ...
    log_message("Cleaning up Trafira runtime after failed start/reload", "info");
    ...
    if (status != 0)
        log_message("Failed to fully clean up Trafira runtime after start/reload failure", "warn");
...
function start() {
    ...
    if (status != 0) {
        log_message("Startup verification failed after Trafira was started; rolling back DNS changes", "warn");
        cleanup_failed_runtime();
        return status;
    }
...
function restart_runtime_for_reload() {
    ...
    log_message("Reload requires a full Trafira runtime restart", "info");
    ...
    if (status != 0) {
        log_message("Reload runtime restart verification failed after Trafira was started; rolling back DNS changes", "fatal");
        cleanup_failed_runtime();
        return status;
    }
...
function reload(reason) {
    ...
    log_message("Reloading Trafira", "info");
    ...
    if (!module_success(STATE_UC, [ "trafira-running", RT_TABLE_NAME, NFT_TABLE_NAME, NFT_FAKEIP_MARK ])) {
        log_message("Runtime state is incomplete; restarting Trafira runtime", "info");
    ...
    let dnsmasq_managed_state = dnsmasq_has_trafira_managed_state() ? 1 : 0;
    ...
    if (as_string(getenv("TRAFIRA_RUNTIME_CACHE_INVALIDATED") || "0") == "1" ||
    ...
    if (plan_result.status != 0) {
        if (plan_result.status == 2) {
            log_message("Reload state is unavailable; restarting Trafira runtime", "info");
            return finish_reload_status(restart_runtime_for_reload(), reload_config_fingerprint);
        }
    ...
    if (plan.needs_sing_box_reload == 1) {
        ...
        if (status != 0) {
            log_message("Reload verification failed after sing-box was reloaded; stopping Trafira runtime", "fatal");
            cleanup_failed_runtime();
            return status;
        }
        status = module_status(PRIORITY_UC, [ "start-runtime" ]);
...
function restart() {
    log_message("Restarting Trafira", "info");
    ...
    log_message("Restart verification failed after Trafira was started; stopping Trafira runtime", "fatal");
    cleanup_failed_runtime();
    return 1;
}
...
function uninstall() {
    log_message("Uninstalling Trafira", "info");
    ...
    if (fs.stat("/etc/init.d/trafira") != null) {
        command_success_from_args([ "/etc/init.d/trafira", "stop" ]);
        command_success_from_args([ "/etc/init.d/trafira", "disable" ]);
    }

    package_manager_remove_if_installed("luci-i18n-trafira-ru");
    package_manager_remove_if_installed("luci-app-trafira");
    ...
    command_success_from_args([ "rm", "-rf", "/usr/lib/trafira" ]);
