...
const SERVICE_NAME = getenv("TRAFIRA_SERVICE_NAME") || "trafira";
const STATE_UC = LIB_DIR + "/service/state.uc";
const UI_UC = LIB_DIR + "/service/ui.uc";
const STATE_DIR = getenv("TRAFIRA_UI_STATE_DIR") || "/var/run/trafira/ui-state";
const PENDING_RELOAD_FILE = getenv("TRAFIRA_PENDING_RELOAD_FILE") || "/var/run/trafira/reload.pending";
const SERVICE_ACTION_DIR = getenv("TRAFIRA_UI_SERVICE_ACTION_DIR") || STATE_DIR + "/service-actions";
const SERVICE_ACTION_LOCK_DIR = getenv("TRAFIRA_UI_SERVICE_ACTION_LOCK_DIR") || STATE_DIR + "/service-actions.lock";
const LATENCY_ACTION_DIR = getenv("TRAFIRA_UI_LATENCY_ACTION_DIR") || STATE_DIR + "/latency-actions";
const COMPONENT_ACTION_DIR = getenv("TRAFIRA_UI_COMPONENT_ACTION_DIR") || getenv("UPDATES_JOB_DIR") || "/var/run/trafira/component-actions";
const SUBSCRIPTION_ACTION_DIR = getenv("TRAFIRA_UI_SUBSCRIPTION_ACTION_DIR") || getenv("TRAFIRA_SUBSCRIPTION_UPDATE_JOB_DIR") || "/var/run/trafira/subscription-update-jobs";
const SING_BOX_VERSION_CACHE_FILE = getenv("TRAFIRA_UI_SING_BOX_VERSION_CACHE_FILE") || STATE_DIR + "/sing-box-version";
const SING_BOX_VERSION_CACHE_LOCK_DIR = getenv("TRAFIRA_UI_SING_BOX_VERSION_CACHE_LOCK_DIR") || SING_BOX_VERSION_CACHE_FILE + ".lock";
const SING_BOX_VARIANT_STATE_FILE = getenv("TRAFIRA_UI_SING_BOX_VARIANT_STATE_FILE") || "/etc/trafira/sing-box-variant";
const SING_BOX_BIN_PATH = getenv("TRAFIRA_UI_SING_BOX_BIN_PATH") || "/usr/bin/sing-box";
const SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS = getenv("TRAFIRA_UI_SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS") || "1";
const SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS = getenv("TRAFIRA_UI_SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS") || "30";
const ACTION_FINISHED_TTL_MINUTES = getenv("TRAFIRA_UI_ACTION_FINISHED_TTL_MINUTES") || "60";
const ACTION_ACKED_TTL_SECONDS = getenv("TRAFIRA_UI_ACTION_ACKED_TTL_SECONDS") || "15";
const ACTION_STALE_GRACE_SECONDS = getenv("TRAFIRA_UI_ACTION_STALE_GRACE_SECONDS") || "15";
const SERVICE_ACTION_TIMEOUT_SECONDS = getenv("TRAFIRA_UI_SERVICE_ACTION_TIMEOUT_SECONDS") || "120";
const SERVICE_ACTION_SETTLE_SECONDS = getenv("TRAFIRA_UI_SERVICE_ACTION_SETTLE_SECONDS") || "2";
const RUNTIME_STABLE_MIN_AGE = getenv("TRAFIRA_RUNTIME_STABLE_MIN_AGE") || "2";
const NFT_TABLE_NAME = getenv("NFT_TABLE_NAME") || "TrafiraTable";
const RT_TABLE_NAME = getenv("RT_TABLE_NAME") || "trafira";
...
function ui_state_json() {
    ...
    let trafira_running = arg_number(ARGV[1]);
    let trafira_enabled = arg_number(ARGV[2]);
    let trafira_status = as_string(ARGV[3]);
    let trafira_dns_configured = arg_number(ARGV[4]);
    let sing_box_running = arg_number(ARGV[5]);
    let sing_box_enabled = arg_number(ARGV[6]);
    let sing_box_status = as_string(ARGV[7]);

    if (trafira_status == "")
        trafira_status = service_status_text(trafira_running, trafira_enabled);
    ...
    write_json({
        service: {
            trafira: {
                running: trafira_running,
                enabled: trafira_enabled,
                status: trafira_status,
                dns_configured: trafira_dns_configured
            },
            sing_box: {
...
function current_ui_state_json() {
    ...
    let trafira_is_running = trafira_running() ? 1 : 0;
    let trafira_is_enabled = service_enabled() ? 1 : 0;
    let sing_box_is_running = trafira_is_running ? 1 : (sing_box_running() ? 1 : 0);
    let sing_box_is_enabled = sing_box_enabled() ? 1 : 0;
    let trafira_status = service_status_text(trafira_is_running, trafira_is_enabled);
    ...
    if (active_action == "start")
        trafira_status = "starting";
    else if (active_action == "stop")
        trafira_status = "stopping";
    else if (active_action == "restart")
        trafira_status = "restarting";
    else if (active_action == "reload")
        trafira_status = "reloading";

    write_json({
        service: {
            trafira: {
                running: trafira_is_running,
                enabled: trafira_is_enabled,
                status: trafira_status,
...
function service_action_reached_expected_state(action) {
    ...
    if (expected < 0)
        return false;
    return expected == 1 ? trafira_running() : !trafira_running();
}
...
function launch_worker(args) {
    ...
    let command = command_env({
        ...
        TRAFIRA_SERVICE_NAME: SERVICE_NAME,
        TRAFIRA_UI_STATE_DIR: STATE_DIR,
        TRAFIRA_UI_SERVICE_ACTION_DIR: SERVICE_ACTION_DIR,
        TRAFIRA_UI_SERVICE_ACTION_LOCK_DIR: SERVICE_ACTION_LOCK_DIR,
        TRAFIRA_UI_LATENCY_ACTION_DIR: LATENCY_ACTION_DIR,
        TRAFIRA_UI_COMPONENT_ACTION_DIR: COMPONENT_ACTION_DIR,
        TRAFIRA_UI_SUBSCRIPTION_ACTION_DIR: SUBSCRIPTION_ACTION_DIR,
        TRAFIRA_UI_SING_BOX_VERSION_CACHE_FILE: SING_BOX_VERSION_CACHE_FILE,
        TRAFIRA_UI_SING_BOX_VERSION_CACHE_LOCK_DIR: SING_BOX_VERSION_CACHE_LOCK_DIR,
        TRAFIRA_UI_SING_BOX_VARIANT_STATE_FILE: SING_BOX_VARIANT_STATE_FILE,
        TRAFIRA_UI_SING_BOX_BIN_PATH: SING_BOX_BIN_PATH,
        TRAFIRA_UI_SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS: SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS,
        TRAFIRA_UI_SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS: SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS,
        TRAFIRA_UI_ACTION_FINISHED_TTL_MINUTES: ACTION_FINISHED_TTL_MINUTES,
        TRAFIRA_UI_ACTION_ACKED_TTL_SECONDS: ACTION_ACKED_TTL_SECONDS,
        TRAFIRA_UI_ACTION_STALE_GRACE_SECONDS: ACTION_STALE_GRACE_SECONDS,
        TRAFIRA_UI_SERVICE_ACTION_TIMEOUT_SECONDS: SERVICE_ACTION_TIMEOUT_SECONDS,
        TRAFIRA_UI_SERVICE_ACTION_SETTLE_SECONDS: SERVICE_ACTION_SETTLE_SECONDS,
...
function run_pending_reload_after_service_action(action, success) {
    ...
    command_success_from_args([ "logger", "-t", SERVICE_NAME, "[info] Applying pending Trafira reload" ]);
...
function finish_service_action_after_command(action, job_id_value, status, spawn_waiter) {
    ...
    if (!service_enabled() && !trafira_running()) {
...
function service_action_worker(path, action, job_id_value, reason) {
    ...
    if (reason != "")
        push(args, reason);
    let command = "TRAFIRA_UI_ACTION_TRACKED=1 " + command_from_args(args) + " >/dev/null 2>&1";
