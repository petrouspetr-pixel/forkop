...
function log_message(message, level) {
    ...
    run_args([ "logger", "-t", "trafira", "[" + level + "] " + as_string(message) ]);
}
...
function safe_rm_rf(path) {
    path = as_string(path);
    if (path == "" || substr(path, 0, length("/var/run/trafira/")) != "/var/run/trafira/")
        return true;
...
function validate_runtime_mark_ranges_context(context) {
    ...
    if (fakeip_mark == null || outbound_mark == null)
        fail_validation("Trafira marks contain invalid numeric constants. Aborted.");
...
function sing_box_variant_marker(ctx, value) {
    ...
    if (path == "")
        path = "/etc/trafira/sing-box-variant";
...
function sing_box_version_state(ctx) {
    ...
    if (path == "")
        path = "/etc/trafira/sing-box-version";
...
function managed_sing_box_service_script(marker) {
    ...
    if (marker == "")
        marker = "Trafira managed sing-box service for binary variants";
...
function install_managed_sing_box_service_script(ctx) {
    let stamp = clock();
    let tmp_file = sprintf("/etc/init.d/sing-box.trafira.%d.%d", stamp[0], stamp[1]);
...
function check_provider_requirements(ctx) {
    ...
    check_provider_requirement(
        ...
        "Failed to prepare the Trafira zapret state directory in " + ctx.zapret_state_dir + ". Aborted."
    );
    ...
    check_provider_requirement(
        ...
        "Failed to prepare the Trafira zapret2 state directory in " + ctx.zapret2_state_dir + ". Aborted."
    );
    ...
    check_provider_requirement(
        ...
        "Failed to prepare the Trafira ByeDPI state directory in " + ctx.byedpi_state_dir + ". Aborted."
    );
}
...
function check_runtime_requirements() {
    ...
    if (dhcp_has_https_dns_proxy_options("/etc/config/dhcp") === true)
        log_message("https-dns-proxy is enabled in DHCP config. Disable it or edit /etc/config/dhcp before starting Trafira.", "error");

    if (has_outbound_section(ctx))
        log_message("Proxy outbound configuration found", "debug");
    else
        log_message("No proxy outbound sections found. Trafira will use direct and/or provider-only routing.", "warn");
