...
const TRAFIRA_RUNTIME_CACHE_FORMAT = int(getenv("TRAFIRA_RUNTIME_CACHE_FORMAT") || "8");
...
let section_cache_dir = TRAFIRA_SECTION_CACHE_DIR;
...
function legacy_metadata_path(section_name) {
    return TRAFIRA_SUBSCRIPTION_METADATA_DIR + "/" + section_name + ".json";
}

function persistent_source_path(source_section, suffix) {
    return TRAFIRA_PERSISTENT_SUBSCRIPTION_CACHE_DIR + "/" + source_section + suffix;
}
...
function new_section_state(section_name) {
    return {
        version: TRAFIRA_RUNTIME_CACHE_FORMAT,
