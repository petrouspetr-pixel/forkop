...
function trafira_config_masked_line(line) {
...
function trafira_config_masked(path) {
    let data = fs.readfile(path);
    if (data == null)
        exit(1);
    ...
    for (let i = 0; i < length(lines); i++) {
        ...
        let masked = trafira_config_masked_line(line);
        print(masked, "\n");
...
function render_global_system_info() {
    let value = object_or_empty(read_stdin_json());
    let trafira_version = object_value(value, "trafira_version") || "unknown";
    ...
    print_line("\ud83d\udd73\ufe0f Trafira:   " + trafira_version);
...
function nft_ruleset_other_mark_lines(table_name) {
    ...
    let in_trafira_table = false;

    for (let line in split(read_stdin(), "\n")) {
        line = as_string(line);
        if (index(line, "table inet " + table_name) >= 0) {
            in_trafira_table = true;
            continue;
        }

        if (match(line, /^table/) != null)
            in_trafira_table = false;

        if (!in_trafira_table && (index(line, "mark set") >= 0 || index(line, "meta mark") >= 0))
            print_line(line);
    }
}
        ...
        if (line_contains(line, needle))
        {
            if (needle == "sing-box" && is_udp_unsupported_outbound_log(line)) {
                if (!udp_http_notice_emitted) {
                    push(filtered, "UDP traffic through HTTP outbounds is not supported by sing-box; repeated UDP warnings for HTTP outbounds are hidden by Trafira.");
...
function render_trafira_logs() {
    ...
    for (let line in lines) {
        if (line == "")
            continue;
        if (!line_contains(line, "trafira") && !line_contains(line, "sing-box"))
            continue;
        ...
        if (line_contains(line, "trafira") && line_contains(line, "Starting Trafira"))
    ...
    print_line("No 'Starting Trafira' message found, showing last 100 lines");
...
function system_info_cache_valid(path, trafira_version, luci_app_version, ttl, now) {
    ...
    return cache.trafira_version == trafira_version && cache.luci_app_version == luci_app_version;
}

function system_info_json() {
    write_json({
        trafira_version: as_string(ARGV[1]),
        trafira_latest_version: as_string(ARGV[2]),
...
else if (mode == "trafira-config-masked")
    trafira_config_masked(ARGV[1]);
