...
const CONFIG_NAME = getenv("TRAFIRA_CONFIG_NAME") || constants.TRAFIRA_CONFIG_NAME || "trafira";
const LIB_DIR = getenv("TRAFIRA_LIB") || "/usr/lib/trafira";
const TRAFIRA_VERSION = getenv("TRAFIRA_VERSION") || constants.TRAFIRA_VERSION || "";
const TRAFIRA_CONFIG = getenv("TRAFIRA_CONFIG") || constants.TRAFIRA_CONFIG || "/etc/config/" + CONFIG_NAME;
const TRAFIRA_SERVICE_NAME = getenv("TRAFIRA_SERVICE_NAME") || constants.TRAFIRA_SERVICE_NAME || "trafira";
const TRAFIRA_RELEASE_REPO = getenv("TRAFIRA_RELEASE_REPO") || constants.TRAFIRA_RELEASE_REPO || "petrouspetr-pixel/trafira";
const TRAFIRA_LUCI_VIEW_DIR = getenv("TRAFIRA_LUCI_VIEW_DIR") || constants.TRAFIRA_LUCI_VIEW_DIR || "/www/luci-static/resources/view/trafira";
...
const SYSTEM_INFO_CACHE_TTL = int(getenv("TRAFIRA_SYSTEM_INFO_CACHE_TTL") || "3600");
...
const RT_TABLE_NAME = getenv("RT_TABLE_NAME") || constants.RT_TABLE_NAME || "trafira";
const NFT_TABLE_NAME = getenv("NFT_TABLE_NAME") || constants.NFT_TABLE_NAME || "TrafiraTable";
const NFT_FAKEIP_MARK = getenv("NFT_FAKEIP_MARK") || constants.NFT_FAKEIP_MARK || "0x04000000";
const NFT_COMMON_SET_NAME = getenv("NFT_COMMON_SET_NAME") || constants.NFT_COMMON_SET_NAME || "trafira_subnets";
const NFT_PORT_SET_NAME = getenv("NFT_PORT_SET_NAME") || constants.NFT_PORT_SET_NAME || "trafira_ports";
const NFT_IP_PORT_SET_NAME = getenv("NFT_IP_PORT_SET_NAME") || constants.NFT_IP_PORT_SET_NAME || "trafira_ip_ports";
const NFT_INTERFACE_SET_NAME = getenv("NFT_INTERFACE_SET_NAME") || constants.NFT_INTERFACE_SET_NAME || "trafira_interfaces";
const NFT_DISCORD_SET_NAME = getenv("NFT_DISCORD_SET_NAME") || constants.NFT_DISCORD_SET_NAME || "trafira_discord_subnets";
...
const SB_VARIANT_STATE_FILE = getenv("SB_VARIANT_STATE_FILE") || constants.SB_VARIANT_STATE_FILE || "/etc/trafira/sing-box-variant";
const SING_BOX_BIN_PATH = getenv("TRAFIRA_DIAGNOSTICS_SING_BOX_BIN_PATH") || "/usr/bin/sing-box";
...
function check_logs() {
    ...
    let rendered = status_capture([ "trafira-logs" ], command_output_from_args([ "logread" ]));
    if (rendered.output != "")
...
function trafira_logs_fixture() {
    let rendered = status_capture([ "trafira-logs" ], read_stdin());
    if (rendered.output != "")
        print(rendered.output);
...
function show_config(visibility) {
    visibility = as_string(visibility || "masked");
    if (!file_exists(TRAFIRA_CONFIG)) {
        nolog("Configuration file not found");
        return 1;
    }
    if (visibility == "raw")
        print(as_string(fs.readfile(TRAFIRA_CONFIG)));
    else
        print(status_output([ "trafira-config-masked", TRAFIRA_CONFIG ], null));
    return 0;
}

function show_version() {
    print(TRAFIRA_VERSION, "\n");
    return 0;
}
...
function get_luci_app_version() {
    let path = TRAFIRA_LUCI_VIEW_DIR + "/main.js";
    ...
    for (let line in split(as_string(data), "\n")) {
        ...
        if (matched != null && matched[1] == "TRAFIRA_LUCI_APP_VERSION")
...
function system_info_cache_is_valid() {
    ...
    if (now > 0 && generated_at > 0 && SYSTEM_INFO_CACHE_TTL > 0 && now - generated_at >= SYSTEM_INFO_CACHE_TTL)
        return false;
    return cache.trafira_version == TRAFIRA_VERSION && cache.luci_app_version == get_luci_app_version();
}
...
function build_system_info() {
    let trafira_latest_version = first_line_value("/tmp/trafira.latest-version.cache", "unknown");
    ...
    return {
        trafira_version: TRAFIRA_VERSION,
        trafira_latest_version: trafira_latest_version || "unknown",
        luci_app_version,
        sing_box_version,
...
function neutralize_zapret_defaults() {
    log_message("Standalone zapret is not neutralized automatically; Trafira uses /opt/zapret/nfq/nfqws as an external provider and manages only its own NFQUEUE range.", "info");
    return 0;
}
...
function dnsmasq_has_trafira_dns() {
    return module_success(DNS_APPLY_UC, [ "has-trafira-dns" ]);
}
...
function get_sing_box_status() {
    ...
    let dns_configured = dnsmasq_has_trafira_dns() ? 1 : 0;
...
function get_status() {
    let running = module_success(SERVICE_STATE_UC, [
        "trafira-stably-running", RT_TABLE_NAME, NFT_TABLE_NAME, NFT_FAKEIP_MARK, RUNTIME_STABLE_MIN_AGE
    ]) ? 1 : 0;
    let enabled = file_executable("/etc/rc.d/S99" + TRAFIRA_SERVICE_NAME) ? 1 : 0;
    let dns_configured = dnsmasq_has_trafira_dns() ? 1 : 0;
    write_service_status(running, enabled, dns_configured);
    return 0;
}
...
function global_check(arg1, arg2) {
    ...
    if (file_executable("/etc/init.d/zapret") && command_success_from_args([ "/etc/init.d/zapret", "status" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret service is active. Trafira uses separate queues, but packet-level policy overlap is possible.");
    }
    else if (file_executable("/etc/init.d/zapret") && command_success_from_args([ "/etc/init.d/zapret", "enabled" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret autostart is enabled. Trafira will not modify /etc/config/zapret.");
    }

    if (file_executable("/etc/init.d/zapret2") && command_success_from_args([ "/etc/init.d/zapret2", "status" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret2 service is active. Trafira uses separate queues, but packet-level policy overlap is possible.");
    }
    else if (file_executable("/etc/init.d/zapret2") && command_success_from_args([ "/etc/init.d/zapret2", "enabled" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret2 autostart is enabled. Trafira will not modify /etc/config/zapret2.");
    }
...
else if (mode == "trafira-logs-fixture")
    exit(trafira_logs_fixture());
