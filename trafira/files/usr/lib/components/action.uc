...
const BIN_PATH = getenv("TRAFIRA_BIN") || constants.TRAFIRA_BIN || "/usr/bin/trafira";
const SERVICE_INIT = getenv("TRAFIRA_SERVICE_INIT") || constants.TRAFIRA_SERVICE_INIT || "/etc/init.d/trafira";
const TRAFIRA_VERSION = getenv("TRAFIRA_VERSION") || constants.TRAFIRA_VERSION || "";
const TRAFIRA_RELEASE_REPO = getenv("TRAFIRA_RELEASE_REPO") || constants.TRAFIRA_RELEASE_REPO || "petrouspetr-pixel/trafira";
...
const SB_MANAGED_SERVICE_MARKER = getenv("SB_MANAGED_SERVICE_MARKER") || constants.SB_MANAGED_SERVICE_MARKER || "Trafira managed sing-box service for binary variants";
...
function init_tmp_dir() {
    ...
    tmp_dir = trim(command_output_from_args([ "mktemp", "-d", "/tmp/trafira-updates.XXXXXX" ]));
    if (tmp_dir == "") {
        tmp_dir = "/tmp/trafira-updates." + owner_pid();
...
function make_tmp_file(prefix) {
    init_tmp_dir();
    let base = tmp_dir != "" ? tmp_dir + "/" + as_string(prefix) + ".XXXXXX" : "/tmp/trafira-updates-" + as_string(prefix) + ".XXXXXX";
...
function restart_trafira_after_failed_sing_box_change() {
    if (!trafira_stopped_for_sing_box_change || !trafira_was_running || !file_exists(SERVICE_INIT))
        return;
    updates_log("Restarting Trafira after failed sing-box component change");
...
function run_logged(description, command, capture_install_error) {
    ...
    if (output_file == "")
        output_file = "/tmp/trafira-updates-command." + owner_pid();
...
function latest_trafira_release_json() {
    let parts = split(TRAFIRA_RELEASE_REPO, "/");
    if (length(parts) != 2 || as_string(parts[0]) == "" || as_string(parts[1]) == "")
        return "";
    return fetch_github_release_json(parts[0], parts[1]);
}

function latest_trafira_version() {
    let response = latest_trafira_release_json();
    if (response == "")
        return "";
    return trim(helper_output_input(response, "object-get-default", [ "tag_name", "" ]));
}

function fetch_trafira_latest_release_metadata() {
    let response = latest_trafira_release_json();
    if (response == "")
        return "";
...
function write_trafira_latest_version_cache(value, timestamp) {
    if (as_string(value) == "")
        return;
    write_file("/tmp/trafira.latest-version.cache", as_string(value) + "\n" + as_string(timestamp) + "\n");
}
...
function clear_version_caches() {
    remove_file("/tmp/trafira.latest-version.cache");
    remove_file(SYSTEM_INFO_CACHE_FILE);
    remove_file("/tmp/trafira/system-info.json");
}
...
function install_managed_sing_box_service_script() {
    let tmp = "/etc/init.d/sing-box.trafira." + owner_pid();
...
function trafira_status_running_with_timeout() {
    init_tmp_dir();
    let output_file = make_tmp_file("trafira-status");
    if (output_file == "")
        return false;
...
function restart_trafira_after_successful_change() {
    if (!file_exists(SERVICE_INIT))
        return;
    if (!trafira_was_running) {
        updates_log("Trafira was not running before component change; restart skipped");
        prepare_sing_box_service_disabled();
        return;
    }
    run_logged("Restarting Trafira after successful component change", command_from_args([ SERVICE_INIT, "restart" ]));
}
...
function stop_trafira_before_sing_box_change() {
    ...
    if (trafira_was_running && file_exists(SERVICE_INIT))
        run_logged("Stopping Trafira before sing-box package change", command_from_args([ SERVICE_INIT, "stop" ]));

    if (trafira_was_running && file_exists(BIN_PATH))
        command_success_from_args([ BIN_PATH, "restore_dnsmasq" ]);

    prepare_sing_box_service_disabled();
}

function wait_trafira_running_after_sing_box_change() {
    if (!trafira_was_running)
        return true;
    ...
    let waited = 0;
    while (waited < 60) {
        if (trafira_status_running_with_timeout()) {
...
function move_file_portable(source_path, target_path) {
    ...
    let staged_path = as_string(target_path) + ".trafira-move." + owner_pid();
...
function install_staged_file(source_path, target_path, mode) {
    let staged_path = as_string(target_path) + ".trafira-new." + owner_pid();
    remove_file(staged_path);
...
function install_sing_box_extended_package(action) {
    ...
    if (current_variant == "extended" || current_variant == "extended-compressed") {
        ...
        if (file_exists("/usr/lib/libcronet.so")) {
            cronet_touched = true;
            backup_cronet = "/usr/lib/libcronet.so.trafira-backup." + owner_pid();
    ...
    if (!wait_trafira_running_after_sing_box_change()) {
        ...
        if (restore_sing_box_after_failed_extended_package_install(current_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, package_file, cronet_touched)) {
            remove_file(backup_binary);
            remove_file(backup_cronet);
            action_fail("sing_box", action, "sing-box-extended package was installed but Trafira did not start cleanly; previous sing-box variant was restored", current_version, latest_version);
        }
        action_fail("sing_box", action, "sing-box-extended package was installed but Trafira did not start cleanly and previous sing-box variant could not be restored", current_version, latest_version);
    }
...
function install_sing_box_extended(action, compressed) {
    ...
    if (!wait_trafira_running_after_sing_box_change()) {
        ...
        if (restore_sing_box_after_failed_extended_install(current_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, archive_file, cronet_touched)) {
            remove_file(backup_binary);
            remove_file(backup_cronet);
            action_fail("sing_box", action, label + " was installed but Trafira did not start cleanly; previous sing-box variant was restored", current_version, latest_version);
        }
        action_fail("sing_box", action, label + " was installed but Trafira did not start cleanly and previous sing-box variant could not be restored", current_version, latest_version);
    }
...
function install_package_sing_box(action, tiny) {
    ...
    if (file_exists("/usr/bin/sing-box")) {
        backup_binary = backup_on_tmpfs ? tmp_dir + "/sing-box.trafira-backup" :
            "/usr/bin/sing-box.trafira-backup." + owner_pid();
        if (!move_file_to_backup("/usr/bin/sing-box", backup_binary))
            action_fail("sing_box", action, "Failed to backup current sing-box binary", current_version, latest_version);
    }
    if (file_exists("/usr/lib/libcronet.so")) {
        cronet_touched = true;
        backup_cronet = backup_on_tmpfs ? tmp_dir + "/libcronet.so.trafira-backup" :
            "/usr/lib/libcronet.so.trafira-backup." + owner_pid();
        if (!move_file_to_backup("/usr/lib/libcronet.so", backup_cronet)) {
...
function check_trafira() {
    let metadata = fetch_trafira_latest_release_metadata();
    ...
    if (latest_version == "unknown")
        action_fail("trafira", "check_update", "Failed to check Trafira updates", TRAFIRA_VERSION, latest_version);

    write_trafira_latest_version_cache(latest_version, now_seconds());
    if (!helper_success("trafira-release-version-valid", [ TRAFIRA_VERSION ])) {
        updates_log("Trafira current version is not a release version (" + TRAFIRA_VERSION + ")");
        action_success("trafira", "check_update", "Installed version is newer than release", TRAFIRA_VERSION, latest_version, 0, "dev", release_url);
    }

    let compare = trim(helper_output("trafira-release-version-compare", [ TRAFIRA_VERSION, latest_version ]));
    if (compare == "")
        action_fail("trafira", "check_update", "Failed to compare Trafira versions", TRAFIRA_VERSION, latest_version);
    let status = status_from_compare(int(compare));
    if (status == "")
        action_fail("trafira", "check_update", "Failed to compare Trafira versions", TRAFIRA_VERSION, latest_version);
    if (status == "latest") {
        updates_log("Trafira is already up to date (" + TRAFIRA_VERSION + ")");
        action_success("trafira", "check_update", "Latest version is installed", TRAFIRA_VERSION, latest_version, 0, status, release_url);
    }
    if (status == "outdated") {
        updates_log("Trafira update found: " + TRAFIRA_VERSION + " -> " + latest_version);
        action_success("trafira", "check_update", "Update is available", TRAFIRA_VERSION, latest_version, 0, status, release_url);
    }
    updates_log("Trafira installed version is newer than upstream release: " + TRAFIRA_VERSION + " -> " + latest_version);
    action_success("trafira", "check_update", "Installed version is newer than release", TRAFIRA_VERSION, latest_version, 0, status, release_url);
}

function resolve_trafira_release(latest_version) {
    let release_json = latest_trafira_release_json();
    if (release_json == "")
        return null;
    let asset_ext = is_apk() ? "apk" : "ipk";
    let i18n_required = pkg_is_installed("luci-i18n-trafira-ru") ? "1" : "0";
    let plan = trim(helper_output_input(release_json, "trafira-release-plan", [ latest_version, asset_ext, i18n_required ]));
...
function install_trafira() {
    let latest_version = latest_trafira_version();
    if (latest_version == "")
        latest_version = "unknown";
    if (latest_version == "unknown")
        action_fail("trafira", "install", "Failed to resolve Trafira release", TRAFIRA_VERSION, latest_version);

    write_trafira_latest_version_cache(latest_version, now_seconds());
    init_tmp_dir() || action_fail("trafira", "install", "Failed to create temporary directory", TRAFIRA_VERSION, latest_version);
    updates_log("Resolving Trafira release " + latest_version + " packages");
    let release = resolve_trafira_release(latest_version);
    if (release == null)
        action_fail("trafira", "install", "Failed to resolve Trafira release packages", TRAFIRA_VERSION, latest_version);
    ...
    restart_trafira_after_successful_change();
    clear_version_caches();
    let new_version = installed_package_version("trafira");
    if (new_version == "")
        new_version = latest_version;
    updates_log("Trafira updated to " + new_version);
...
else if (mode == "latest-trafira-version")
    print(latest_trafira_version(), "\n");
else if (mode == "trafira-release-metadata")
    print(fetch_trafira_latest_release_metadata(), "\n");
else if (mode == "tmp-download-capacity-fixture")
    exit(tmp_download_capacity_ok(ARGV[1], ARGV[2], ARGV[3], ARGV[4]) ? 0 : 1);
else {
    warn("Usage: components/action.uc <component-action|latest-trafira-version|trafira-release-metadata|tmp-download-capacity-fixture> ...\n");
    exit(1);
}
