...
VALIDATOR="$ROOT_DIR/trafira/files/usr/lib/providers/zapret/validator.uc"
ZAPRET2_VALIDATOR="$ROOT_DIR/trafira/files/usr/lib/providers/zapret2/validator.uc"
WORK_DIR="$(mktemp -d)"
...
valid_nfqws="$(ucode -L "$TRAFIRA_LIB" -- "$VALIDATOR" validate-json nfqws '--dpi-desync=fake --dpi-desync-repeats 2')"
assert_json_field "$valid_nfqws" valid true

configured="$(ucode -L "$TRAFIRA_LIB" -- "$VALIDATOR" strategy-or-default "$(printf -- '--dpi-desync=fake\t--dpi-desync-repeats 2')" '--default')"
[ "$configured" = "--dpi-desync=fake --dpi-desync-repeats 2" ] ||
  fail "configured zapret strategy should be normalized, got '$configured'"

defaulted="$(ucode -L "$TRAFIRA_LIB" -- "$VALIDATOR" strategy-or-default "" "$(printf -- '--default\t1')")"
[ "$defaulted" = "--default 1" ] ||
  fail "empty zapret strategy should use normalized default, got '$defaulted'"

if invalid_nfqws="$(ucode -L "$TRAFIRA_LIB" -- "$VALIDATOR" validate-json nfqws '--hostlist domains.txt' 2>/dev/null)"; then
...
if ucode -L "$TRAFIRA_LIB" -- "$VALIDATOR" validate nfqws '--qnum=200' >"$validator_output" 2>/dev/null; then
  fail "qnum override should be rejected for nfqws"
fi
grep -q 'NFQUEUE number is assigned by Trafira' "$validator_output" ||
  fail "qnum rejection should explain ownership"

valid_nfqws2="$(ucode -L "$TRAFIRA_LIB" -- "$ZAPRET2_VALIDATOR" validate-json nfqws2 '--name trafira --intercept=1')"
assert_json_field "$valid_nfqws2" valid true

if invalid_nfqws2="$(ucode -L "$TRAFIRA_LIB" -- "$ZAPRET2_VALIDATOR" validate-json nfqws2 '--intercept=0' 2>/dev/null)"; then
...
if ucode -L "$TRAFIRA_LIB" -- "$VALIDATOR" validate-json nfqws2 '--name trafira --intercept=1' >/dev/null 2>&1; then
  fail "zapret validator must not own nfqws2 validation"
fi
if ucode -L "$TRAFIRA_LIB" -- "$ZAPRET2_VALIDATOR" validate-json nfqws '--dpi-desync=fake' >/dev/null 2>&1; then
...
let valid_nfqws2 = zapret2_validator.validate_strategy("nfqws2", "--name trafira --intercept=1", "");
if (!valid_nfqws2.valid)
    exit(1);

if (validator.validate_strategy("nfqws2", "--name trafira --intercept=1", "").valid)
    exit(1);
...
ucode -L "$TRAFIRA_LIB" "$WORK_DIR/require-zapret-validator.uc"
