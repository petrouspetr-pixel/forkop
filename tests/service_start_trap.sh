...
INITD_UC="$ROOT_DIR/trafira/files/usr/lib/service/initd.uc"
...
SUBSCRIPTION_CACHE_UC="$ROOT_DIR/trafira/files/usr/lib/subscription/cache.uc"
NFQUEUE_RUNTIME_UC="$ROOT_DIR/trafira/files/usr/lib/providers/nfqueue/runtime.uc"
BYEDPI_RUNTIME_UC="$ROOT_DIR/trafira/files/usr/lib/providers/byedpi/runtime.uc"
PRIORITY_UC="$ROOT_DIR/trafira/files/usr/lib/singbox/priority.uc"
...
reject_pattern() {
  ...
  ! grep -Fq "$pattern" "$TRAFIRA_BIN" || fail "$label"
}
...
[ -r "$TRAFIRA_BIN" ] || fail "trafira binary source is missing"
[ -r "$TRAFIRA_BIN" ] || fail "trafira entrypoint is missing"
...
reject_pattern "trap " \
  "trafira shell entrypoint must not own startup fail-safe traps"
reject_pattern "clear_startup_failsafe_trap" \
  "trafira shell entrypoint must not keep old trap helper"
...
require_file_pattern "$TRAFIRA_INIT" 'TRAFIRA_LAST_START_STATUS="$?"' \
  "init.d start_service must preserve backend start status for rc.common"
require_file_pattern "$TRAFIRA_INIT" 'service_started()' \
