...
UI_UC="$ROOT_DIR/trafira/files/usr/lib/service/ui.uc"
...
UI_RUNTIME_SH="$TRAFIRA_FILES/usr/lib/ui_runtime.sh"
WORK_DIR="$(mktemp -d)"
...
ui_ucode() {
  ucode -L "$TRAFIRA_FILES/usr/lib" "$UI_UC" "$@"
}
...
if grep -R -n "ui_runtime.sh" "$TRAFIRA_FILES" >/dev/null 2>&1; then
  fail "runtime files must not reference ui_runtime.sh"
fi

ui_runtime_shell_symbols='ui_runtime_|trafira_fast_get_ui_state|TRAFIRA_UI_RUNTIME|load_ui_runtime'
if grep -R -n -E "$ui_runtime_shell_symbols" "$TRAFIRA_BIN" "$TRAFIRA_INIT" "$TRAFIRA_FILES/usr/lib" --include='*.sh' >/dev/null 2>&1; then
  fail "ui_runtime shell symbols must not remain in runtime shell"
fi

if grep -R -n '^get_ui_capabilities()' "$TRAFIRA_FILES/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
grep -Fq '"trafira-stably-running"' "$UI_UC" ||
  fail "UI Trafira status must use stable runtime state to avoid crash-loop flicker"
...
TRAFIRA_UI_ACTION_FINISHED_TTL_MINUTES=60 ui_ucode cleanup-action-dir-fixture "$cleanup_dir"
[ ! -e "$old_state" ] ||
  fail "old finished action state should be cleaned without find"

export TRAFIRA_UI_STATE_DIR="$WORK_DIR/ui-state"
export TRAFIRA_UI_SERVICE_ACTION_DIR="$TRAFIRA_UI_STATE_DIR/service-actions"
export TRAFIRA_UI_SERVICE_ACTION_LOCK_DIR="$TRAFIRA_UI_STATE_DIR/service-actions.lock"
export TRAFIRA_UI_LATENCY_ACTION_DIR="$TRAFIRA_UI_STATE_DIR/latency-actions"
export TRAFIRA_UI_COMPONENT_ACTION_DIR="$TRAFIRA_UI_STATE_DIR/component-actions"
export TRAFIRA_UI_SUBSCRIPTION_ACTION_DIR="$TRAFIRA_UI_STATE_DIR/subscription-actions"

latency_start="$(
  TRAFIRA_BIN=/bin/true \
...
latency_async_state="$TRAFIRA_UI_LATENCY_ACTION_DIR/$latency_job_id.json"
...
service_state="$TRAFIRA_UI_SERVICE_ACTION_DIR/$job_id.json"
...
TRAFIRA_UI_SERVICE_ACTION_TIMEOUT_SECONDS=1 \
TRAFIRA_UI_SERVICE_ACTION_SETTLE_SECONDS=0 \
TRAFIRA_SERVICE_INIT=/bin/true \
TRAFIRA_BIN=/bin/true \
TRAFIRA_LIB="$TRAFIRA_FILES/usr/lib" \
...
component_job="$TRAFIRA_UI_COMPONENT_ACTION_DIR/component-1.json"
mkdir -p "$TRAFIRA_UI_COMPONENT_ACTION_DIR"
