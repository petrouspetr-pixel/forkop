...
CLI_UC="$TRAFIRA_BIN"
UPDATER="$ROOT_DIR/trafira/files/usr/lib/components/updater.uc"
UPDATES_UC="$ROOT_DIR/trafira/files/usr/lib/components/updates.uc"
ACTION_UC="$ROOT_DIR/trafira/files/usr/lib/components/action.uc"
WORK_DIR="$(mktemp -d)"
...
[ ! -e "$ROOT_DIR/trafira/files/usr/lib/updater.sh" ] ||
...
grep -Fq 'trafira_status_running_with_timeout()' "$ACTION_UC" ||
  fail "components/action.uc must use bounded Trafira status checks for component actions"
...
grep -Fq 'fail_package_sing_box_install(action, tiny, "was installed, but Trafira did not start cleanly"' "$ACTION_UC" ||
...
status_timeout_line="$(grep -n '^function trafira_status_running_with_timeout()' "$ACTION_UC" | cut -d: -f1)"
capture_state_line="$(grep -n '^function capture_trafira_running_state()' "$ACTION_UC" | cut -d: -f1)"
[ -n "$status_timeout_line" ] && [ -n "$capture_state_line" ] && [ "$status_timeout_line" -lt "$capture_state_line" ] ||
  fail "components/action.uc must declare bounded status helper before capture_trafira_running_state for OpenWrt ucode"
...
grep -Fq 'backup_binary = backup_on_tmpfs ? tmp_dir + "/sing-box.trafira-backup"' "$ACTION_UC" ||
...
TRAFIRA_LIB="$package_runtime_lib" \
TRAFIRA_RUNTIME_STATE_DIR="$WORK_DIR/package-runtime" \
...
PATH="$package_runtime_bin:$PATH" \
TRAFIRA_LIB="$package_runtime_lib" \
TRAFIRA_RUNTIME_STATE_DIR="$WORK_DIR/apk-package-runtime" \
TRAFIRA_BIN="$WORK_DIR/missing-trafira" \
TRAFIRA_SERVICE_INIT="$WORK_DIR/missing-init" \
...
release_json="$(cat <<'JSON'
{
  ...
  "assets": [
    {"name": "trafira_1.2.3.ipk", "browser_download_url": "https://example.com/backend.ipk"},
    {"name": "luci-app-trafira_1.2.3.ipk", "browser_download_url": "https://example.com/app.ipk"},
    {"name": "luci-i18n-trafira-ru_1.2.3.ipk", "browser_download_url": "https://example.com/i18n.ipk"}
  ]
}
JSON
)"

assert_eq "$(printf 'https://example.com/release\ttrafira_1.2.3.ipk\thttps://example.com/backend.ipk\tluci-app-trafira_1.2.3.ipk\thttps://example.com/app.ipk\tluci-i18n-trafira-ru_1.2.3.ipk\thttps://example.com/i18n.ipk')" \
  "$(printf '%s' "$release_json" | ucode "$UPDATER" trafira-release-plan 1.2.3 ipk 1)" \
  "trafira release plan with i18n"
assert_eq "$(printf 'https://example.com/release\ttrafira_1.2.3.ipk\thttps://example.com/backend.ipk\tluci-app-trafira_1.2.3.ipk\thttps://example.com/app.ipk\t\t')" \
  "$(printf '%s' "$release_json" | ucode "$UPDATER" trafira-release-plan 1.2.3 ipk 0)" \
  "trafira release plan without i18n"
if printf '%s' "$release_json" | ucode "$UPDATER" trafira-release-plan 9.9.9 ipk 0 >/dev/null 2>&1; then
  fail "trafira release plan should reject tag mismatch"
fi
...
if printf '%s' "$missing_i18n_json" | ucode "$UPDATER" trafira-release-plan 1.2.3 ipk 1 >/dev/null 2>&1; then
  fail "trafira release plan should require i18n asset when requested"
fi

ucode "$UPDATER" trafira-release-version-valid 1.2.3 ||
  fail "three-part Trafira release version should be valid"
for invalid_version in v1.2.3 1.2 1.2.3.4 1.2.3-1 1.2.3-r1; do
  if ucode "$UPDATER" trafira-release-version-valid "$invalid_version" >/dev/null 2>&1; then
    fail "invalid Trafira release version was accepted: $invalid_version"
  fi
done
assert_eq "-1" "$(ucode "$UPDATER" trafira-release-version-compare 1.2.3 1.2.4)" \
  "three-part Trafira release version comparison"
...
cp "$TRAFIRA_LIB/core/uci.uc" "$fake_lib/core/uci.uc"
cp "$TRAFIRA_LIB/core/common.uc" "$fake_lib/core/common.uc"
...
start_json="$(TRAFIRA_LIB="$fake_lib" UPDATES_JOB_DIR="$component_actions_dir" \
  ucode -L "$TRAFIRA_LIB" "$UPDATES_UC" component-action-async sing_box check_update)"
...
for _ in 1 2 3 4 5; do
  status_json="$(TRAFIRA_LIB="$fake_lib" UPDATES_JOB_DIR="$component_actions_dir" \
    ucode -L "$TRAFIRA_LIB" "$UPDATES_UC" component-action-status "$job_id")"
...
start_json="$(TRAFIRA_LIB="$fake_lib" UPDATES_JOB_DIR="$component_actions_dir" \
  ucode -L "$TRAFIRA_LIB" "$UPDATES_UC" component-action-async sing-box check_update)"
job_id="$(JSON_VALUE="$start_json" node - <<'NODE'
const value = JSON.parse(process.env.JSON_VALUE);
if (!value.success || !value.job_id) {
  process.exit(1);
...
ucode -L "$TRAFIRA_LIB" "$ACTION_UC" tmp-download-capacity-fixture \
  31457280 100663296 100663296 33554432 ||
  fail "sufficient tmpfs and MemAvailable capacity must pass"

if ucode -L "$TRAFIRA_LIB" "$ACTION_UC" tmp-download-capacity-fixture \
