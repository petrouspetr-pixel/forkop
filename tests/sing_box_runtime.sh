...
CLI_UC="$TRAFIRA_BIN"
SING_BOX_RUNTIME_SH="$TRAFIRA_LIB/sing_box_runtime.sh"
LIFECYCLE_UC="$TRAFIRA_LIB/service/lifecycle.uc"
SINGBOX_RUNTIME_UC="$TRAFIRA_LIB/singbox/runtime.uc"
SINGBOX_GENERATOR_UC="$TRAFIRA_LIB/singbox/generator.uc"
WORK_DIR="$(mktemp -d)"
...
if grep -R -n -E 'sing_box_runtime_ucode|rulesets_ucode|sing_box_configure_service|sing_box_init_config|get_service_listen_address|get_device_ipv4_address|get_download_detour_tag' "$TRAFIRA_BIN" "$TRAFIRA_LIB" --include='*.sh' >/dev/null 2>&1; then
...
grep -Fq 'TRAFIRA_RULE_CONDITION_CACHE_DIR' "$LIFECYCLE_UC" ||
...
stable_variant="$({
  PATH="$WORK_DIR/bin:$PATH" \
    ...
    ucode -L "$TRAFIRA_LIB" "$SINGBOX_RUNTIME_UC" variant
...
if PATH="$WORK_DIR/check-bin:$PATH" \
  ucode -L "$TRAFIRA_LIB" "$SINGBOX_RUNTIME_UC" check-config-fixture \
...
generator_reason="$(ucode -L "$TRAFIRA_LIB" "$SINGBOX_RUNTIME_UC" generator-failure-reason-fixture \
...
ucode -L "$TRAFIRA_LIB" "$SINGBOX_RUNTIME_UC" save-config-file-fixture \
...
if ucode -L "$TRAFIRA_LIB" "$SINGBOX_GENERATOR_UC" generate-config-fixture \
...
trafira.settings.dns_server=1.1.1.1
trafira.settings.bootstrap_dns_server=1.1.1.1
trafira.settings.config_path=/tmp/sing-box/config.json
trafira.settings.cache_path=/tmp/sing-box/cache.db
trafira.settings.log_level=warn
trafira.uci_proxy=section
trafira.uci_proxy.enabled=1
trafira.uci_proxy.action=connection
trafira.uci_proxy.outbound_jsons={"type":"direct"}
trafira.uci_proxy.domain_suffix=example.org
EOF_UCI
TRAFIRA_UCI_STATE_FILE="$WORK_DIR/generator-uci.state" \
  TRAFIRA_SECTION_CACHE_DIR="$WORK_DIR/generated-from-uci.section-cache" \
  ucode -L "$TRAFIRA_LIB" "$SINGBOX_GENERATOR_UC" generate-config "$WORK_DIR/generated-from-uci.json" "127.0.0.1" "0"
