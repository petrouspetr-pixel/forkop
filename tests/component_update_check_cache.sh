...
UPDATES_UC="$TRAFIRA_LIB/components/updates.uc"
WORK_DIR="$(mktemp -d)"
...
updates_ucode() {
  TEST_COMPONENT_UPDATE_CHECK_ENABLED="${TEST_COMPONENT_UPDATE_CHECK_ENABLED:-1}" \
    ...
    TRAFIRA_COMPONENT_UPDATE_CHECK_CACHE_DIR="$cache_dir" \
    TRAFIRA_COMPONENT_UPDATE_CHECK_STATE_FILE="$timestamp_file" \
...
[ -s "$cache_dir/trafira.json" ] ||
  fail "automatic checks must cache the Trafira result"
[ -s "$timestamp_file" ] ||
