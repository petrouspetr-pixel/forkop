...
RULE_CONFIG="$TRAFIRA_LIB/config/rule.uc"
...
LIFECYCLE="$TRAFIRA_LIB/service/lifecycle.uc"
...
[ ! -e "$TRAFIRA_LIB/config_validation.sh" ] ||
  fail "config_validation.sh shell owner must be removed"

if grep -R -n "config_validation.sh" "$TRAFIRA_FILES" >/dev/null 2>&1; then
  fail "runtime files must not reference config_validation.sh"
fi

legacy_symbols='(^|[^A-Za-z0-9_])(config_validate_runtime|check_requirements|commit_trafira_config|mwan3_is_active|get_inline_remote_ruleset_format|detect_inline_ruleset_reference_kind)([^A-Za-z0-9_]|$)'
if grep -R -n -E "$legacy_symbols" "$TRAFIRA_BIN" "$TRAFIRA_LIB" --include='*.sh' >/dev/null 2>&1; then
