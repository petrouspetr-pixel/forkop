...
eval "$(ucode -L "$TRAFIRA_LIB" "$TRAFIRA_LIB/core/constants.uc" shell-env)"
...
TRAFIRA_LIB="$TRAFIRA_LIB" ucode -L "$TRAFIRA_LIB" "$MIGRATION" migrate-fixture "$WORK_DIR/fixture.json" podkop >"$WORK_DIR/output.json"
...
assert(out.removed_caches.includes('/var/run/trafira/section-cache/legacy-sub.json'), 'section cache removal');
NODE
...
trafira.settings.routing_excluded_ips=192.0.2.0/24
trafira.legacy=rule
trafira.legacy.enabled=1
trafira.legacy.connection_type=proxy
trafira.legacy.proxy_config_type=url
trafira.legacy.proxy_string=vless://one
trafira.old_direct=section
trafira.old_direct.enabled=1
trafira.old_direct.action=direct
trafira.vpn=section
trafira.vpn.enabled=1
trafira.vpn.connection_type=vpn
trafira.vpn.interface=awg0
trafira.vpn.domain_resolver_enabled=1
trafira.vpn.domain_resolver_dns_type=doh
trafira.vpn.domain_resolver_dns_server=https://dns.example/dns-query
EOF_UCI
mkdir -p "$WORK_DIR/runtime" "$WORK_DIR/persistent-cache"
: >"$WORK_DIR/runtime-migrate.log"
TRAFIRA_UCI_STATE_FILE="$WORK_DIR/runtime-migrate.state" \
TRAFIRA_UCI_LOG_FILE="$WORK_DIR/runtime-migrate.log" \
TRAFIRA_CONFIG_NAME="trafira" \
TMP_SUBSCRIPTION_FOLDER="$WORK_DIR/tmp-subscriptions" \
TRAFIRA_RUNTIME_STATE_DIR="$WORK_DIR/runtime" \
TRAFIRA_PERSISTENT_SUBSCRIPTION_CACHE_DIR="$WORK_DIR/persistent-cache" \
TRAFIRA_INTERNAL_CONFIG_TRIGGER_GUARD="$WORK_DIR/internal-config-change" \
ucode -L "$TRAFIRA_LIB" "$MIGRATION" migrate-podkop

grep -Fxq 'trafira.legacy=section' "$WORK_DIR/runtime-migrate.state" ||
  fail "runtime migration must convert legacy rule type through core.uci"
grep -Fxq 'trafira.legacy.action=connection' "$WORK_DIR/runtime-migrate.state" ||
  fail "runtime migration must write migrated action through core.uci"
grep -Fxq 'trafira.old_direct.action=bypass' "$WORK_DIR/runtime-migrate.state" ||
  fail "runtime migration must convert direct action to bypass through core.uci"
grep -Eq '^trafira\\.cfg[0-9a-f]+=$' "$WORK_DIR/runtime-migrate.state" && fail "anonymous fixture section should include a type"
grep -Fxq 'trafira.legacy.selector_proxy_links=vless://one' "$WORK_DIR/runtime-migrate.state" ||
...
grep -Eq '^trafira\.cfg[0-9a-f]+=section_interface$' "$WORK_DIR/runtime-migrate.state" ||
  fail "runtime migration must create an anonymous interface settings section"
grep -Eq '^trafira\.cfg[0-9a-f]+\.section=vpn$' "$WORK_DIR/runtime-migrate.state" ||
  fail "runtime interface settings section must reference its parent"
grep -Eq '^trafira\.cfg[0-9a-f]+\.name=' "$WORK_DIR/runtime-migrate.state" ||
  fail "runtime interface settings section must store the interface name"
if grep -Fq 'trafira.legacy.urltest_enabled=' "$WORK_DIR/runtime-migrate.state"; then
  fail "runtime migration must delete migrated urltest_enabled through core.uci"
fi
if grep -Fq 'trafira.settings.routing_excluded_ips=' "$WORK_DIR/runtime-migrate.state"; then
  fail "runtime migration must delete removed routing_excluded_ips through core.uci"
fi
if grep -Fq 'trafira.legacy.proxy_string=' "$WORK_DIR/runtime-migrate.state"; then
  fail "runtime migration must delete legacy proxy_string through core.uci"
fi
grep -Fxq 'commit trafira' "$WORK_DIR/runtime-migrate.log" ||
