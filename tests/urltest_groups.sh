...
CACHE_UC="$TRAFIRA_LIB/subscription/cache.uc"
WORK_DIR="$(mktemp -d)"
...
normalize_subscription() {
  local input="$1"
  local output="$2"
  ucode -L "$TRAFIRA_LIB" "$PARSER_UC" normalize-content "$input" "$output"
}
...
ucode -L "$TRAFIRA_LIB" "$CACHE_UC" get-outbound-metadata "$xray_config.section-cache" proxy "$WORK_DIR/missing-outbound-metadata.json" >"$xray_metadata"
...
for (let outbound in value.outbounds || [])
    flags[outbound.tag] = {
        allow: outbound.__trafira_allow_group === true,
        hidden: outbound.__trafira_hidden === true
    };
...
ucode -L "$TRAFIRA_LIB" "$CACHE_UC" get-outbound-metadata "$singbox_config.section-cache" proxy "$WORK_DIR/missing-outbound-metadata.json" >"$singbox_metadata"
