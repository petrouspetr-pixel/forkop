...
DIAGNOSTICS="$ROOT_DIR/trafira/files/usr/lib/diagnostics/status.uc"
DIAGNOSTICS_RUNTIME="$ROOT_DIR/trafira/files/usr/lib/diagnostics/runtime.uc"
...
status_ucode() {
  ucode -L "$TRAFIRA_LIB" "$DIAGNOSTICS" "$@"
}
...
[ "$(TRAFIRA_VERSION=runtime-test ucode -L "$TRAFIRA_LIB" "$DIAGNOSTICS_RUNTIME" show-version)" = "runtime-test" ] ||
...
grep -Fq '"trafira-stably-running", RT_TABLE_NAME, NFT_TABLE_NAME, NFT_FAKEIP_MARK, RUNTIME_STABLE_MIN_AGE' "$DIAGNOSTICS_RUNTIME" ||
  fail "diagnostics Trafira status must use stable runtime state to avoid crash-loop flicker"
...
capabilities="$(
  TRAFIRA_DIAGNOSTICS_SING_BOX_BIN_PATH="$WORK_DIR/missing-sing-box" \
  TRAFIRA_LIB="$TRAFIRA_LIB" \
    ucode -L "$TRAFIRA_LIB" "$DIAGNOSTICS_RUNTIME" get-server-capabilities
)"
...
masked_config="$WORK_DIR/trafira-masked"
...
EOF
masked_output="$(status_ucode trafira-config-masked "$masked_config")"
case "$masked_output" in
  *device-secret*|*vless://secret*|*token=secret*|*user:password*) fail "masked Trafira config leaked a secret" ;;
esac
case "$masked_output" in
  *"option hwid 'MASKED'"*) ;;
  *) fail "masked Trafira config must preserve the HWID option shape" ;;
esac
case "$masked_output" in
  *"option url 'MASKED'"*) ;;
  *) fail "masked Trafira config must mask subscription section URLs" ;;
esac
...
{
  printf 'Tue Jun 30 11:00:00 2026 user.notice trafira: [info] Starting Trafira\n'
  ...
  printf 'Tue Jun 30 11:01:00 2026 user.notice trafira: [info] large logread marker survived stdin transport\n'
} >"$WORK_DIR/large-logread.txt"
large_logs="$(TRAFIRA_LIB="$TRAFIRA_LIB" ucode -L "$TRAFIRA_LIB" "$DIAGNOSTICS_RUNTIME" trafira-logs-fixture <"$WORK_DIR/large-logread.txt")" ||
...
trafira.settings.latency_test_url=https://latency.example/generate_204
EOF
FAKE_CURL_LOG="$WORK_DIR/fake-curl.log" \
TRAFIRA_UCI_STATE_FILE="$uci_state" \
TRAFIRA_LIB="$TRAFIRA_LIB" \
PATH="$fake_bin:$PATH" \
  ucode -L "$TRAFIRA_LIB" "$DIAGNOSTICS_RUNTIME" clash-api get_proxy_latency proxy-out 5000 >/dev/null ||
...
PATH="$fake_bin:$PATH" \
  ucode -L "$TRAFIRA_LIB" "$DIAGNOSTICS_RUNTIME" clash-api get_proxy_latencies '["urltest","proxy-a","provider-urltest","proxy-b"]' 5000 "$latency_state" >/dev/null ||
...
printf '%s\n' "$sing_box_netstat" |
  TRAFIRA_LIB="$TRAFIRA_LIB" ucode -L "$TRAFIRA_LIB" "$DIAGNOSTICS_RUNTIME" sing-box-standard-ports-listening-fixture >/dev/null ||
  fail "sing-box standard listeners should satisfy diagnostics"
if printf '%s\n' "$sing_box_netstat" | sed '/0.0.0.0:1602/d' |
  TRAFIRA_LIB="$TRAFIRA_LIB" ucode -L "$TRAFIRA_LIB" "$DIAGNOSTICS_RUNTIME" sing-box-standard-ports-listening-fixture >/dev/null 2>&1; then
