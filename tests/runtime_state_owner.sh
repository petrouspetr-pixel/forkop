...
TRAFIRA_INIT="$ROOT_DIR/trafira/files/etc/init.d/trafira"
CLI_UC="$TRAFIRA_BIN"
RUNTIME_STATE="$ROOT_DIR/trafira/files/usr/lib/runtime_state.sh"
UPDATES_RUNTIME="$ROOT_DIR/trafira/files/usr/lib/updates_runtime.sh"
LIFECYCLE_UC="$ROOT_DIR/trafira/files/usr/lib/service/lifecycle.uc"
UPDATES_UC="$ROOT_DIR/trafira/files/usr/lib/components/updates.uc"
STATE_UC="$ROOT_DIR/trafira/files/usr/lib/service/state.uc"
NFT_UC="$ROOT_DIR/trafira/files/usr/lib/nft/apply.uc"
...
grep -Fq 'runtime_state.sh' "$TRAFIRA_BIN" &&
  fail "trafira must not source runtime_state.sh"

service_predicates='process_age_seconds|sing_box_service_pid|pid_is_sing_box|sing_box_service_is_running|sing_box_service_is_stable|trafira_runtime_network_is_configured|trafira_is_running|trafira_is_stably_running|wait_for_trafira_stable_start'
if grep -R -n -E "$service_predicates" "$TRAFIRA_BIN" "$ROOT_DIR/trafira/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
if grep -R -n -F 'updates_runtime.sh' "$TRAFIRA_BIN" "$ROOT_DIR/trafira/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
grep -Fq 'mode == "trafira-running"' "$STATE_UC" ||
  fail "service/state.uc must own Trafira running predicate"
grep -Fq 'mode == "sing-box-service-stable"' "$STATE_UC" ||
  fail "service/state.uc must expose stable sing-box predicate"
grep -Fq 'mode == "wait-trafira-stable-start"' "$STATE_UC" ||
...
grep -Fq '"wait-trafira-stable-start"' "$LIFECYCLE_UC" ||
...
early_stable_line="$(awk -v start="$sing_box_start_line" 'NR > start && /"wait-trafira-stable-start"/ { print NR; exit }' "$LIFECYCLE_UC")"
...
grep -Fq 'Reload verification failed after sing-box was reloaded; stopping Trafira runtime' "$LIFECYCLE_UC" ||
  fail "service/lifecycle.uc must fail reload when sing-box does not stay stable"
grep -Fq 'Reload runtime restart verification failed after Trafira was started; rolling back DNS changes' "$LIFECYCLE_UC" ||
...
if grep -Fq 'nft-create-full-runtime-from-uci' "$LIFECYCLE_UC"; then
  fail "trafira start/reload must rebuild nft runtime instead of appending to stale state"
fi
