...
reject_runtime_pattern() {
  ...
  if rg -n --fixed-strings "$pattern" "$ROOT_DIR/trafira/files" >/dev/null 2>&1; then
    rg -n --fixed-strings "$pattern" "$ROOT_DIR/trafira/files" >&2
    fail "$label"
  fi
}
...
reject_runtime_regex() {
  ...
  if rg -n "$pattern" "$ROOT_DIR/trafira/files" >/dev/null 2>&1; then
    rg -n "$pattern" "$ROOT_DIR/trafira/files" >&2
    fail "$label"
  fi
}
...
reject_runtime_pattern "Temporary sing-box config hash:" \
  "sing-box config logs must not emit duplicate hash lines"
reject_runtime_pattern "Trafira did not reach a stable running state after start" \
