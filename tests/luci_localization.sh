...
SOURCE_PO="$ROOT_DIR/fe-app-trafira/locales/trafira.ru.po"
PACKAGE_PO="$ROOT_DIR/luci-app-trafira/po/ru/trafira.po"
SOURCE_POT="$ROOT_DIR/fe-app-trafira/locales/trafira.pot"
PACKAGE_POT="$ROOT_DIR/luci-app-trafira/po/templates/trafira.pot"
CALLS_JSON="$ROOT_DIR/fe-app-trafira/locales/calls.json"
...
if grep -Fq '_("Dismiss")' "$SECTION_JS"; then
  fail "Trafira modals must use Close instead of the shared LuCI Dismiss key"
fi
grep -Fq '_("Close")' "$SECTION_JS" ||
  fail "Trafira section settings modal must expose a Close action"
