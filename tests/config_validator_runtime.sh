...
JS
  TRAFIRA_LIB="$TRAFIRA_LIB" ucode -L "$TRAFIRA_LIB" "$VALIDATOR" validate-runtime-fixture "$normalized" "$context"
}
...
JSON
if output="$(TRAFIRA_LIB="$TRAFIRA_LIB" ucode -L "$TRAFIRA_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/bad-byedpi.json" "$provider_context" 2>/dev/null)"; then
...
mkdir -p "$runtime_lib"
ln -s "$TRAFIRA_LIB/core" "$runtime_lib/core"
ln -s "$TRAFIRA_LIB/config" "$runtime_lib/config"
ln -s "$TRAFIRA_LIB/singbox" "$runtime_lib/singbox"
ln -s "$TRAFIRA_LIB/subscription" "$runtime_lib/subscription"
ln -s "$TRAFIRA_LIB/providers" "$runtime_lib/providers"
