...
export TRAFIRA_FAKE_INIT_CAPTURE="$WORK_DIR/pending-reload-init.args"
...
state_ucode() {
  ucode -L "$TRAFIRA_LIB" "$STATE_UC" "$@"
}

nft_ucode() {
  ucode -L "$TRAFIRA_LIB" "$NFT_UC" "$@"
}
...
if ! PATH="$WORK_DIR/stable-start-bin:$PATH" \
  ...
  SING_BOX_TEST_PID_FILE="$WORK_DIR/sing-box.pid" \
  SING_BOX_TEST_NETSTAT_FILE="$WORK_DIR/sing-box.netstat" \
  state_ucode trafira-running trafira TrafiraTable 0x00100000; then
  kill "$sing_box_pid" >/dev/null 2>&1 || true
  wait "$sing_box_pid" 2>/dev/null || true
  fail "stable-start fixture must expose configured Trafira networking"
fi
...
if PATH="$WORK_DIR/stable-start-bin:$PATH" \
  ...
  state_ucode trafira-running trafira TrafiraTable 0x00100000 >/dev/null 2>&1; then
  kill "$sing_box_pid" >/dev/null 2>&1 || true
  wait "$sing_box_pid" 2>/dev/null || true
...
if ! PATH="$WORK_DIR/stable-start-bin:$PATH" \
  ...
  SING_BOX_TEST_PID_FILE="$WORK_DIR/sing-box.pid" \
  SING_BOX_TEST_NETSTAT_FILE="$WORK_DIR/sing-box.netstat" \
  state_ucode wait-trafira-stable-start trafira TrafiraTable 0x00100000 2 2; then
  kill "$sing_box_pid" >/dev/null 2>&1 || true
  wait "$sing_box_pid" 2>/dev/null || true
...
cat >"$WORK_DIR/service-dns-state.json" <<'JSON'
{
  ...
  "dhcp_dnsmasq": {
    ...
    "noresolv": "1",
    "cachesize": "0",
    "trafira_server": [ "127.0.0.42#53" ],
    "trafira_noresolv": "0",
    "trafira_cachesize": "1500"
  },
...
0
[dhcp.@dnsmasq[0].trafira_server]
127.0.0.42#53
[dhcp.@dnsmasq[0].trafira_noresolv]
0
[dhcp.@dnsmasq[0].trafira_cachesize]
1500
[dhcp.trafira.present]
1
EOF_DNSMASQ_SIG
