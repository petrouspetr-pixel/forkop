...
export TRAFIRA_DEADLINE_HELPER_PATH="$deadline_helper"
export TRAFIRA_INSTALLER_DEADLINE_HELPER="$deadline_helper"
export TRAFIRA_INSTALLER_COMMAND_RESULT="$WORK_DIR/installer-command"
...
grep -Fq '/usr/bin/trafira component_action sing_box "$action"' "$INSTALLER" ||
  fail "selected sing-box install must delegate to trafira component_action"
...
if grep -n -E 'rm -f /var/luci-indexcache|rm -f /tmp/luci-indexcache|/etc/init\.d/rpcd[[:space:]]+reload|/etc/init\.d/trafira[[:space:]]+(start|stop|disable|enable|restart)|/etc/init\.d/trafira[[:space:]]+(stop|disable)' "$INSTALLER" >/dev/null 2>&1; then
...
release_json='{"tag_name":"0.0.1","assets":[{"name":"trafira_0.0.1.ipk","browser_download_url":"https://example.com/trafira.ipk"}]}'
printf '%s' "$release_json" | ucode "$helper" release-asset-url backend ipk | grep -Fxq 'https://example.com/trafira.ipk' ||
  fail "embedded helper must resolve the exact three-part Trafira package name"
if printf '%s' '{"tag_name":"0.0.1","assets":[{"name":"trafira_0.0.1_all.ipk","browser_download_url":"https://example.com/old.ipk"}]}' |
  ucode "$helper" release-asset-url backend ipk | grep -q .; then
  fail "embedded helper must reject package names outside the Trafira release format"
fi
...
case "$1" in
  ...
  remove)
    ...
    printf '%s\n' "$1" >> "$TRAFIRA_INSTALLER_OPKG_LOG"
    [ "${TRAFIRA_INSTALLER_FAIL_REMOVE:-}" != "$1" ] || exit 1
    ;;
esac
exit 0
SH
...
#!/usr/bin/env sh
printf '%s\n' "$1" >> "$TRAFIRA_INSTALLER_INIT_LOG"
case "$1" in
  enabled|status|running)
    printf '%s\n' "$$" >> "$TRAFIRA_INSTALLER_HANG_PID_LOG"
    trap '' TERM
    sh -c '
      trap "" TERM
      printf "%s\n" "$$" >> "$TRAFIRA_INSTALLER_HANG_PID_LOG"
...
TRAFIRA_INSTALLER_INIT="$WORK_DIR/hanging-init" \
TRAFIRA_INSTALLER_BIN="$WORK_DIR/hanging-bin" \
TRAFIRA_INSTALLER_LIB="$WORK_DIR/missing-trafira-lib" \
TRAFIRA_INSTALLER_UCI_DEFAULTS="$WORK_DIR/missing-uci-defaults" \
TRAFIRA_INSTALLER_LUCI_VIEW="$WORK_DIR/missing-luci-view" \
TRAFIRA_INSTALLER_MENU_JSON="$WORK_DIR/missing-menu.json" \
TRAFIRA_INSTALLER_ACL_JSON="$WORK_DIR/missing-acl.json" \
TRAFIRA_INSTALLER_RU_LMO="$WORK_DIR/missing-ru.lmo" \
...
TRAFIRA_INSTALLER_ORPHAN_PPID="$ORPHAN_PROBE_PPID" \
TRAFIRA_INSTALLER_SERVICE_PROBE_TIMEOUT=1 \
TRAFIRA_INSTALLER_SERVICE_ACTION_TIMEOUT=3 \
TRAFIRA_INSTALLER_INIT_LOG="$WORK_DIR/hanging-init.log" \
TRAFIRA_INSTALLER_HANG_PID_LOG="$HANG_PID_LOG" \
...
grep -Fxq 'TRAFIRA_WAS_ENABLED=1' "$WORK_DIR/hanging-state.env" ||
...
: >"$WORK_DIR/init.log"
TRAFIRA_INSTALLER_INIT="$WORK_DIR/trafira-init" \
TRAFIRA_INSTALLER_RPCD_INIT="$WORK_DIR/missing-rpcd" \
TRAFIRA_INSTALLER_LUCI_CACHE_GLOBS="$WORK_DIR/luci-indexcache*" \
TRAFIRA_INSTALLER_LATEST_VERSION_CACHE="$WORK_DIR/latest.cache" \
TRAFIRA_INSTALLER_SYSTEM_INFO_CACHE="$WORK_DIR/system-info.json" \
TRAFIRA_INSTALLER_SERVER_COUNTRY_CACHE="$WORK_DIR/server-country.json" \
TRAFIRA_INSTALLER_SING_BOX_VERSION_CACHE="$WORK_DIR/sing-box-version" \
TRAFIRA_INSTALLER_TMP_SYSTEM_INFO_CACHE="$WORK_DIR/tmp-system-info.json" \
TRAFIRA_WAS_ENABLED=1 \
TRAFIRA_WAS_RUNNING=1 \
TRAFIRA_INSTALLER_INIT_LOG="$WORK_DIR/init.log" \
