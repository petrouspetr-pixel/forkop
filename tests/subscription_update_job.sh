...
SH
chmod +x "$WORK_DIR/bin/trafira"

ASYNC_ENV=(
  "TRAFIRA_LIB=$ROOT_DIR/trafira/files/usr/lib"
  "TRAFIRA_BIN=$WORK_DIR/bin/trafira"
...
async_response="$(env "${ASYNC_ENV[@]}" ucode -L "$ROOT_DIR/trafira/files/usr/lib" "$UPDATES_UC" subscription-update-async proxy 1)"
...
for _ in $(seq 1 30); do
  final_status="$(env "${ASYNC_ENV[@]}" ucode -L "$ROOT_DIR/trafira/files/usr/lib" "$UPDATES_UC" subscription-update-status "$job_id")"
