...
ZAPRET_RUNTIME="$TRAFIRA_LIB/providers/zapret/runtime.uc"
ZAPRET2_RUNTIME="$TRAFIRA_LIB/providers/zapret2/runtime.uc"
ZAPRET_COMMON="$TRAFIRA_LIB/providers/zapret/common.uc"
ZAPRET2_COMMON="$TRAFIRA_LIB/providers/zapret2/common.uc"
NFQUEUE_RUNTIME="$TRAFIRA_LIB/providers/nfqueue/runtime.uc"
NFQUEUE_CHECK="$TRAFIRA_LIB/providers/nfqueue/check.uc"
NFQUEUE_VALIDATOR="$TRAFIRA_LIB/providers/nfqueue/validator.uc"
ZAPRET2_CHECK="$TRAFIRA_LIB/providers/zapret2/check.uc"
ZAPRET2_VALIDATOR="$TRAFIRA_LIB/providers/zapret2/validator.uc"
...
[ ! -e "$TRAFIRA_LIB/zapret.sh" ] ||
  fail "zapret.sh shell owner must be removed"
[ ! -e "$TRAFIRA_LIB/zapret2.sh" ] ||
...
if grep -R -n -E 'providers\.zapret\.|providers/zapret/' "$TRAFIRA_LIB/providers/zapret2" >/dev/null 2>&1; then
...
status_json="$(TRAFIRA_CONFIG_NAME=trafira-definitely-missing ucode -L "$TRAFIRA_LIB" "$ZAPRET_RUNTIME" status)"
...
check_json="$(ucode -L "$TRAFIRA_LIB" "$ZAPRET2_RUNTIME" check)"
...
ucode -L "$TRAFIRA_LIB" -- "$ZAPRET2_VALIDATOR" validate-json nfqws2 '--name trafira --intercept=1' >/dev/null ||
...
printf 'table inet x { chain y { queue num 4301 bypass } }\n' |
  ucode -L "$TRAFIRA_LIB" "$ZAPRET2_CHECK" nft-queue-overlap TrafiraTable 4300 4555 >/dev/null ||
