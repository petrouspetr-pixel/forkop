...
INITD_UC="$TRAFIRA_LIB/service/initd.uc"
STATE_UC="$TRAFIRA_LIB/service/state.uc"
INITD="$ROOT_DIR/trafira/files/etc/init.d/trafira"
WORK_DIR="$(mktemp -d)"
...
initd_ucode() {
  ucode -L "$TRAFIRA_LIB" "$INITD_UC" "$@"
}

config_file="$WORK_DIR/trafira"
...
if TRAFIRA_PENDING_RELOAD_FILE="$pending_file" \
...
retry_service="$WORK_DIR/trafira-init"
cat >"$retry_service" <<EOF
#!/bin/sh
printf '%s\n' "\$1" >>"$retry_call_file"
EOF
chmod +x "$retry_service"
TRAFIRA_SERVICE_INIT="$retry_service" \
  TRAFIRA_START_RETRY_PID_FILE="$retry_pid_file" \
  TRAFIRA_START_RETRY_DELAY_SECONDS=1 \
...
if grep -E -n 'TRAFIRA_(CONFIG_FILE|RELOAD_LOCK_DIR|RUNTIME_STATE_DIR|PENDING_RELOAD_FILE|SERVICE_TRIGGER_SYNC_FILE|INTERNAL_CONFIG_TRIGGER_GUARD|CONFIG_CHANGE_REASON|BIN|SERVICE_INIT)=' "$INITD" >/dev/null 2>&1; then
...
if grep -Fq 'TRAFIRA_STATE_UC' "$INITD" || grep -Fq 'TRAFIRA_UI_UC' "$INITD" || grep -Fq 'TRAFIRA_STATUS_UC' "$INITD"; then
  fail "init.d must not orchestrate state/UI/status modules directly"
fi
if grep -Fq 'procd_set_param command "$TRAFIRA_BIN" start' "$INITD"; then
