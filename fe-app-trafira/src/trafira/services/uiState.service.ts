...
type UiActionMap = Partial<Trafira.UiState['actions']>;
...
function getEmptyUpdatesActions(): StoreType['updatesActions'] {
  return {
    trafiraCheck: { loading: false },
    trafiraInstall: { loading: false },
...
function applyServiceState(uiState: Trafira.UiState) {
  ...
  store.set({
    servicesInfoWidget: {
      ...
      data: {
        ...
        trafiraRunning: uiState.service.trafira.running,
        trafiraEnabled: uiState.service.trafira.enabled,
        trafiraStatus: uiState.service.trafira.status,
      },
    },
...
function applyActionState(actions: UiActionMap = {}) {
  ...
  const latencyProgressSections: Record<string, Trafira.LatencyActionProgress> =
    {};
...
export function applyUiStateToStore(uiState: Trafira.UiState) {
