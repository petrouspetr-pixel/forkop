...
type RuntimeUiStateListener = (uiState: Trafira.UiState) => void;

let runtimeUiStateRefreshPromise: Promise<Trafira.UiState | undefined> | null =
  null;
let lastRuntimeUiStateRefreshAt = 0;
let lastRuntimeUiState: Trafira.UiState | undefined;
...
function hasRunningAction(uiState: Trafira.UiState) {
...
function notifyRuntimeUiStateListeners(uiState: Trafira.UiState) {
...
export async function refreshRuntimeUiState({
  force = false,
}: { force?: boolean } = {}): Promise<Trafira.UiState | undefined> {
  ...
  const promise = TrafiraShellMethods.getUiState()
    .then((response) => {
      if (!response.success) {
        return undefined;
      }
