const LOG_NOTIFICATION_STORAGE_KEY = 'trafira:shown-log-error-notifications:v1';
const MAX_STORED_LOG_NOTIFICATIONS = 500;

export type TrafiraLogNotification =
...
export function getTrafiraLogNotification(
  line: string,
): TrafiraLogNotification | null {
  if (isErrorLogLine(line)) {
    return { kind: 'error', line };
  }

  const update = line.match(
    /\[component-update\]\s+(trafira|sing_box|zapret|zapret2|byedpi)\s+(\S+)/i,
  );
...
export class LogNotificationDeduper {
  ...
  shouldNotify(line: string) {
    if (!getTrafiraLogNotification(line)) {
      return false;
    }
