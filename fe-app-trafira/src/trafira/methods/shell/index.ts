...
import { ClashAPI, Trafira } from '../../types';
...
const COMPONENT_ACTION_STATE_DIR = '/var/run/trafira/component-actions';
...
function parseComponentActionOutput(output: string) {
  return parseJsonObjectOutput<Trafira.ComponentActionResult>(output);
}
...
function parseComponentActionStartResult(
  ...
  return parsedResponse as unknown as Trafira.ComponentActionStartResult;
}

function parseSubscriptionUpdateStartResult(
  response: Awaited<ReturnType<typeof executeShellCommand>>,
) {
  return parseJsonObjectOutput<Trafira.SubscriptionUpdateStartResult>(
    response.stdout,
  );
}
...
function parseSubscriptionUpdateJobState(
  ...
  return parseJsonObjectOutput<Trafira.SubscriptionUpdateJobState>(
    response.stdout,
  );
}
...
function parseUiActionStartResult(
  ...
  return parseJsonObjectOutput<Trafira.UiActionStartResult>(response.stdout);
}

function parseServiceActionState(
  response: Awaited<ReturnType<typeof executeShellCommand>>,
) {
  return parseJsonObjectOutput<Trafira.ServiceActionState>(response.stdout);
}

function parseLatencyActionState(
  response: Awaited<ReturnType<typeof executeShellCommand>>,
) {
  return parseJsonObjectOutput<Trafira.LatencyActionState>(response.stdout);
}
...
async function readTrafiraVersion() {
...
async function isComponentActionStillRunning(
  jobId: string,
  component: Trafira.ComponentName,
  action: Trafira.ComponentAction,
) {
  const response = await callBaseMethod<Trafira.UiState>(
    Trafira.AvailableMethods.GET_UI_STATE,
    [],
    '/usr/bin/trafira',
...
function componentActionFailure(
  response: Awaited<ReturnType<typeof executeShellCommand>>,
  parsedResponse?: Pick<Trafira.ComponentActionResult, 'message'> | null,
) {
  return {
    success: false,
    error: parsedResponse?.message || response.stderr || _('Failed to execute'),
  } as Trafira.MethodFailureResponse;
}
...
export const TrafiraShellMethods = {
  checkDNSAvailable: async () =>
    callBaseMethod<Trafira.DnsCheckResult>(
      Trafira.AvailableMethods.CHECK_DNS_AVAILABLE,
    ),
  checkFakeIP: async () =>
    callBaseMethod<Trafira.FakeIPCheckResult>(
      Trafira.AvailableMethods.CHECK_FAKEIP,
    ),
  checkNftRules: async () =>
    callBaseMethod<Trafira.NftRulesCheckResult>(
      Trafira.AvailableMethods.CHECK_NFT_RULES,
    ),
  checkZapretRuntime: async () =>
    callBaseMethod<Trafira.ZapretCheckResult>(
      Trafira.AvailableMethods.CHECK_ZAPRET_RUNTIME,
    ),
  checkZapret2Runtime: async () =>
    callBaseMethod<Trafira.Zapret2CheckResult>(
      Trafira.AvailableMethods.CHECK_ZAPRET2_RUNTIME,
    ),
  checkByedpiRuntime: async () =>
    callBaseMethod<Trafira.ByedpiCheckResult>(
      Trafira.AvailableMethods.CHECK_BYEDPI_RUNTIME,
    ),
  checkInboundsConfig: async () =>
    callBaseMethod<Trafira.InboundsConfigCheckResult>(
      Trafira.AvailableMethods.CHECK_INBOUNDS_CONFIG,
    ),
  getStatus: async () =>
    callBaseMethod<Trafira.GetStatus>(Trafira.AvailableMethods.GET_STATUS),
  getOutboundMetadata: async (section: string) =>
    callBaseMethod<Trafira.GetOutboundMetadata>(
      Trafira.AvailableMethods.GET_OUTBOUND_METADATA,
      [section],
    ),
  getSubscriptionMetadata: async (section: string) =>
    callBaseMethod<
      Trafira.SubscriptionMetadata | Trafira.SubscriptionMetadata[]
    >(Trafira.AvailableMethods.GET_SUBSCRIPTION_METADATA, [section]),
  checkSingBox: async () =>
    callBaseMethod<Trafira.SingBoxCheckResult>(
      Trafira.AvailableMethods.CHECK_SING_BOX,
    ),
  checkInbounds: async () =>
    callBaseMethod<Trafira.InboundsCheckResult>(
      Trafira.AvailableMethods.CHECK_INBOUNDS,
    ),
  getSingBoxStatus: async () =>
    callBaseMethod<Trafira.GetSingBoxStatus>(
      Trafira.AvailableMethods.GET_SING_BOX_STATUS,
    ),
  getZapretStatus: async () =>
    callBaseMethod<Trafira.GetZapretStatus>(
      Trafira.AvailableMethods.GET_ZAPRET_STATUS,
    ),
  getZapret2Status: async () =>
    callBaseMethod<Trafira.GetZapret2Status>(
      Trafira.AvailableMethods.GET_ZAPRET2_STATUS,
    ),
  getByedpiStatus: async () =>
    callBaseMethod<Trafira.GetByedpiStatus>(
      Trafira.AvailableMethods.GET_BYEDPI_STATUS,
    ),
  getClashApiProxies: async () =>
    callBaseMethod<ClashAPI.Proxies>(Trafira.AvailableMethods.CLASH_API, [
      Trafira.AvailableClashAPIMethods.GET_PROXIES,
    ]),
  getClashApiConnections: async () =>
    callBaseMethod<unknown>(Trafira.AvailableMethods.CLASH_API, [
      Trafira.AvailableClashAPIMethods.GET_CONNECTIONS,
    ]),
  getClashApiProxyLatency: async (tag: string, timeout = '5000') =>
    callBaseMethod<Trafira.GetClashApiProxyLatency>(
      Trafira.AvailableMethods.CLASH_API,
      [Trafira.AvailableClashAPIMethods.GET_PROXY_LATENCY, tag, timeout],
    ),
  getClashApiProxyLatencies: async (tags: string[]) =>
    callBaseMethod<Trafira.GetClashApiProxyLatencies>(
      Trafira.AvailableMethods.CLASH_API,
      [
        Trafira.AvailableClashAPIMethods.GET_PROXY_LATENCIES,
        JSON.stringify(tags),
        '5000',
      ],
    ),
  getClashApiGroupLatency: async (tag: string) =>
    callBaseMethod<Trafira.GetClashApiGroupLatency>(
      Trafira.AvailableMethods.CLASH_API,
      [Trafira.AvailableClashAPIMethods.GET_GROUP_LATENCY, tag, '10000'],
    ),
  setClashApiGroupProxy: async (group: string, proxy: string) =>
    callBaseMethod<unknown>(Trafira.AvailableMethods.CLASH_API, [
      Trafira.AvailableClashAPIMethods.SET_GROUP_PROXY,
      group,
      proxy,
    ]),
  closeClashApiConnection: async (connectionId: string) =>
    callBaseMethod<unknown>(Trafira.AvailableMethods.CLASH_API, [
      Trafira.AvailableClashAPIMethods.CLOSE_CONNECTION,
      connectionId,
    ]),
  closeAllClashApiConnections: async () =>
    callBaseMethod<unknown>(Trafira.AvailableMethods.CLASH_API, [
      Trafira.AvailableClashAPIMethods.CLOSE_ALL_CONNECTIONS,
    ]),
  enable: async () =>
    callBaseMethod<unknown>(
      Trafira.AvailableMethods.ENABLE,
      [],
      '/etc/init.d/trafira',
    ),
  disable: async () =>
    callBaseMethod<unknown>(
      Trafira.AvailableMethods.DISABLE,
      [],
      '/etc/init.d/trafira',
    ),
  globalCheck: async (masked = true) =>
    callBaseMethod<unknown>(Trafira.AvailableMethods.GLOBAL_CHECK, [
      masked ? 'masked' : 'raw',
    ]),
  showSingBoxConfig: async (masked = true) =>
    callBaseMethod<unknown>(Trafira.AvailableMethods.SHOW_SING_BOX_CONFIG, [
      masked ? 'masked' : 'raw',
    ]),
  checkLogs: async () =>
    callBaseMethod<unknown>(Trafira.AvailableMethods.CHECK_LOGS),
  checkSingBoxLogs: async () =>
    callBaseMethod<unknown>(Trafira.AvailableMethods.CHECK_SING_BOX_LOGS),
  getSystemInfo: async () =>
  ...
  getUiCapabilities: async () =>
    callBaseMethod<Trafira.GetUiCapabilities>(
      Trafira.AvailableMethods.GET_UI_CAPABILITIES,
    ),
  getUiState: async () =>
    callBaseMethod<Trafira.UiState>(
      Trafira.AvailableMethods.GET_UI_STATE,
      [],
      '/usr/bin/trafira',
      { timeout: GET_UI_STATE_RPC_TIMEOUT_MS },
    ),
  serviceActionStart: async (action: Trafira.ServiceAction) => {
    const response = await executeShellCommand({
      command: '/usr/bin/trafira',
      args: [Trafira.AvailableMethods.SERVICE_ACTION_ASYNC, action],
    ...
    return {
      success: true,
      data: parsedResponse,
    } as Trafira.MethodSuccessResponse<Trafira.UiActionStartResult>;
  },
  serviceActionStatus: async (jobId: string) => {
    const response = await executeShellCommand({
      command: '/usr/bin/trafira',
      args: [Trafira.AvailableMethods.SERVICE_ACTION_STATUS, jobId],
      timeout: UI_ACTION_RPC_TIMEOUT_MS,
    });
    ...
    return {
      success: true,
      data: parsedResponse,
    } as Trafira.MethodSuccessResponse<Trafira.ServiceActionState>;
  },
  waitServiceActionJob: async (jobId: string, startedAt = Date.now()) => {
    while (Date.now() - startedAt < SERVICE_ACTION_TIMEOUT_MS) {
      ...
      const response = await TrafiraShellMethods.serviceActionStatus(jobId);
  ...
  latencyTestStart: async (
    latencyType: Trafira.LatencyActionState['latency_type'],
    section: string,
    tag: string,
    timeout?: string,
  ) => {
    const response = await executeShellCommand({
      command: '/usr/bin/trafira',
      args: [
        Trafira.AvailableMethods.LATENCY_TEST_ASYNC,
        latencyType,
        section,
        tag,
        ...(timeout ? [timeout] : []),
  ...
  latencyTestStatus: async (jobId: string) => {
    const response = await executeShellCommand({
      command: '/usr/bin/trafira',
      args: [Trafira.AvailableMethods.LATENCY_TEST_STATUS, jobId],
      timeout: UI_ACTION_RPC_TIMEOUT_MS,
    });
    ...
    return {
      success: true,
      data: parsedResponse,
    } as Trafira.MethodSuccessResponse<Trafira.LatencyActionState>;
  },
  waitLatencyTestJob: async (jobId: string, startedAt = Date.now()) => {
    ...
    while (Date.now() - startedAt < LATENCY_TEST_TIMEOUT_MS) {
      ...
      const response = await TrafiraShellMethods.latencyTestStatus(jobId);
  ...
  uiActionAck: async (
    ...
    const response = await executeShellCommand({
      command: '/usr/bin/trafira',
      args: [Trafira.AvailableMethods.UI_ACTION_ACK, kind, jobId],
      timeout: UI_ACTION_RPC_TIMEOUT_MS,
    });
    const parsedResponse = parseUiActionStartResult(response);
  ...
  componentActionStart: async (
    ...
    const response = await executeShellCommand({
      command: '/usr/bin/trafira',
      args: [
        Trafira.AvailableMethods.COMPONENT_ACTION_ASYNC,
        component,
        action,
      ],
      timeout: COMPONENT_ACTION_RPC_TIMEOUT_MS,
    ...
    return {
      success: true,
      data: parsedResponse,
    } as Trafira.MethodSuccessResponse<Trafira.ComponentActionStartResult>;
  },
  componentActionStatus: async (jobId: string) => {
    const response = await executeShellCommand({
      command: '/usr/bin/trafira',
      args: [Trafira.AvailableMethods.COMPONENT_ACTION_STATUS, jobId],
      timeout: COMPONENT_ACTION_RPC_TIMEOUT_MS,
    });
    const parsedResponse = parseComponentActionResult(response);
    ...
    return {
      success: true,
      data: parsedResponse,
    } as Trafira.MethodSuccessResponse<Trafira.ComponentActionResult>;
  },
  componentUpdateCheckCache: async () =>
    callBaseMethod<Trafira.ComponentUpdateCheckCache>(
      Trafira.AvailableMethods.COMPONENT_UPDATE_CHECK_CACHE,
    ),
  waitComponentActionJob: async (
    ...
    while (true) {
      ...
      if ((statusResponse.code ?? 0) !== 0 || !parsedResponse) {
        ...
        if (component === 'trafira' && action === 'install') {
          const installedVersion = expectedLatestVersion
            ? await readTrafiraVersion()
            : '';
            ...
            if (
              ...
              return {
                ...
                data: {
                  ...
                  component,
                  action,
                  message: translate('Trafira has been installed'),
  ...
  subscriptionUpdateStart: async (section?: string, sourceIndex?: number) => {
    const startArgs = [
      Trafira.AvailableMethods.SUBSCRIPTION_UPDATE_ASYNC,
      ...(section ? [section] : []),
    ...
    return {
      success: true,
      data: parsedResponse,
    } as Trafira.MethodSuccessResponse<Trafira.SubscriptionUpdateStartResult>;
  },
  subscriptionUpdateStatus: async (jobId: string) => {
    const response = await executeShellCommand({
      command: '/usr/bin/trafira',
      args: [Trafira.AvailableMethods.SUBSCRIPTION_UPDATE_STATUS, jobId],
      timeout: SUBSCRIPTION_UPDATE_RPC_TIMEOUT_MS,
    });
    ...
    return {
      success: true,
      data: parsedResponse,
    } as Trafira.MethodSuccessResponse<Trafira.SubscriptionUpdateJobState>;
  },
  waitSubscriptionUpdateJob: async (jobId: string) => {
    ...
    while (true) {
      ...
      const response =
        await TrafiraShellMethods.subscriptionUpdateStatus(jobId);
