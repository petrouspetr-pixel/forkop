...
describe('TrafiraShellMethods.latencyAction', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it('keeps waiting through a transient RPC reply loss', async () => {
    ...
    const responsePromise = TrafiraShellMethods.waitLatencyTestJob('job-1');
  ...
  it('passes an optional timeout to latency jobs', async () => {
    ...
    await TrafiraShellMethods.latencyTestStart(
      'proxy',
      'AWG',
      'AWG-out',
      '10000',
