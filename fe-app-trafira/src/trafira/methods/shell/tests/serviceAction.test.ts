...
describe('TrafiraShellMethods.serviceAction', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it('keeps failed finished service state available to low-level waiters', async () => {
    ...
    const responsePromise = TrafiraShellMethods.waitServiceActionJob('job-1');
  ...
  it('returns the backend service action start error message', async () => {
    ...
    await expect(
      TrafiraShellMethods.serviceActionStart('restart'),
    ).resolves.toEqual({
      success: false,
