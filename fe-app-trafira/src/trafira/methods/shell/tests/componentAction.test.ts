...
import { TrafiraShellMethods } from '../index';

describe('TrafiraShellMethods.componentAction', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it('does not fail Trafira self-update when status polling disappears after package replacement', async () => {
    ...
    const responsePromise = TrafiraShellMethods.waitComponentActionJob(
      'job-1',
      'trafira',
      'install',
      '0.7.17.11',
    ...
    await expect(responsePromise).resolves.toEqual({
      ...
      data: {
        ...
        component: 'trafira',
        action: 'install',
        message: 'Trafira has been installed',
  ...
  it('returns the backend component action start error message', async () => {
    ...
    await expect(
      TrafiraShellMethods.componentActionStart('zapret', 'install'),
    ).resolves.toEqual({
      success: false,
