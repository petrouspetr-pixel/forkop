...
import { TrafiraShellMethods } from '../shell';

interface IGetDashboardSectionsResponse {
  success: boolean;
  data: Trafira.OutboundGroup[];
}
...
const DASHBOARD_SECTION_CACHE_DIR = '/var/run/trafira/section-cache';
const CLASH_API_FETCH_TIMEOUT_MS = 5000;

function getDisplayName(section: Trafira.ConfigSection) {
  return section.label || section['.name'];
}

function getSettingsSection(configSections: Trafira.ConfigSection[]) {
  return configSections.find((section) => section['.type'] === 'settings');
}

function getClashApiSecret(configSections: Trafira.ConfigSection[]) {
...
async function getClashApiProxies(
  configSections: Trafira.ConfigSection[],
): Promise<Trafira.MethodResponse<ClashAPI.Proxies>> {
  ...
  return TrafiraShellMethods.getClashApiProxies();
}
...
function ownedChildSections(
  parent: Trafira.ConfigSection,
  children: Trafira.ConfigSection[],
) {
  return children.filter(
    (section): section is Trafira.ConfigSection =>
...
function childSectionsByOwner(
  children: Trafira.ConfigSection[],
  ownerKey: keyof Trafira.ConfigSection,
  ownerValue: string,
) {
...
function hydrateConfigSections(configSections: Trafira.ConfigSection[]) {
  ...
  return configSections.map((section) => {
    ...
    const next: Trafira.ConfigSection = { ...section };
...
function getManualProxyLinks(section: Trafira.ConfigSection) {
  return getListValues(section.selector_proxy_links);
}

function getConnectionInterfaces(section: Trafira.ConfigSection) {
  const values = getListValues(section.interfaces);
  return values.length ? values : getListValues(section.interface);
}

function getJsonOutbounds(section: Trafira.ConfigSection) {
...
function hasSubscriptionSources(section: Trafira.ConfigSection) {
  return getSubscriptionSourceCount(section) > 0;
}

function getSubscriptionSourceCount(section: Trafira.ConfigSection) {
  return getListValues(section.subscription_urls).length;
}

function shouldSortByLatency(section: Trafira.ConfigSection) {
  return section.sort_by_latency === '1';
}

function hasConfiguredUrlTestList(section: Trafira.ConfigSection) {
  return getListValues(section.urltests).length > 0;
}

function hasConfiguredPriorityList(section: Trafira.ConfigSection) {
  return getListValues(section.priority_groups).length > 0;
}

function getUrlTestIds(section: Trafira.ConfigSection) {
...
function isUrlTestEnabled(section: Trafira.ConfigSection) {
  return getUrlTestIds(section).length > 0;
}

function shouldUseProxyGroup(section: Trafira.ConfigSection) {
  return (
...
function getSectionProxyConfigType(section: Trafira.ConfigSection) {
...
function getJsonOutboundDisplayName(section: Trafira.ConfigSection) {
  try {
...
function buildManualLinkByCode(section: Trafira.ConfigSection) {
...
function isSelectorOutbound(outbound: Trafira.Outbound) {
...
function sortUrlTestMembers(outbounds: Trafira.UrlTestMember[]) {
  return outbounds
    .map((outbound, index) => ({ outbound, index }))
    .sort((left, right) => {
...
function getUrlTestConfigs(section: Trafira.ConfigSection): UrlTestConfig[] {
...
function getPriorityGroupIds(section: Trafira.ConfigSection) {
  return getListValues(section.priority_groups);
}

function getPriorityConfigs(section: Trafira.ConfigSection): PriorityConfig[] {
...
function metadataMatchesCurrentSource(
  sectionName: string,
  sourceCount: number,
  metadata: Trafira.SubscriptionMetadata,
) {
  const legacyMetadata = metadata as Trafira.SubscriptionMetadata & {
    source_index?: number;
