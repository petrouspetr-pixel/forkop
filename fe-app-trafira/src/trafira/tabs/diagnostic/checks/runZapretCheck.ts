...
export async function runZapretCheck() {
  ...
  const zapretStatus = await TrafiraShellMethods.getZapretStatus();
  ...
  const items: Array<IDiagnosticsChecksItem> = [
    ...
    },
    {
      ...
      key: hasZapretRules
        ? trafiraRuntimeReady
          ? _('Trafira-managed nfqws runtime is ready')
          : _('Trafira-managed nfqws runtime is not ready')
        : unexpectedRuntime
          ? _('Unexpected Trafira-managed nfqws runtime is running')
          : _('Trafira-managed nfqws runtime is not running'),
    ...
    },
    {
      ...
      key: standaloneServiceRunning
        ? hasZapretRules
          ? _('Standalone Zapret is active together with Trafira Zapret rules')
