...
export async function runZapret2Check() {
  ...
  const zapret2Status = await TrafiraShellMethods.getZapret2Status();
  ...
  const items: Array<IDiagnosticsChecksItem> = [
    ...
    {
      state: unexpectedRuntime || !trafiraRuntimeReady ? 'error' : 'success',
      key: hasZapret2Rules
        ? trafiraRuntimeReady
          ? _('Trafira-managed nfqws2 runtime is ready')
          : _('Trafira-managed nfqws2 runtime is not ready')
        : unexpectedRuntime
          ? _('Unexpected Trafira-managed nfqws2 runtime is running')
          : _('Trafira-managed nfqws2 runtime is not running'),
    ...
    },
    {
      ...
      key: standaloneServiceRunning
        ? hasZapret2Rules
          ? _(
              'Standalone Zapret2 is active together with Trafira Zapret2 rules',
            )
