...
function serverPrefix(item: Trafira.InboundCheckItem) {
  return `${item.label}:`;
}

function formatListen(item: Trafira.InboundCheckItem) {
  return `${item.listen}:${Number(item.listen_port || 0)} [${item.required_proto}]`;
}

function getPortConflictItem(
  item: Trafira.InboundCheckItem,
...
export async function runInboundsCheck() {
  ...
  const inboundsChecks = await TrafiraShellMethods.checkInbounds();
