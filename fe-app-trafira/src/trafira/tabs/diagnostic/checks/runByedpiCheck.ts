...
export async function runByedpiCheck() {
  ...
  const byedpiStatus = await TrafiraShellMethods.getByedpiStatus();
  ...
  const items: Array<IDiagnosticsChecksItem> = [
    ...
    {
      state:
        unexpectedRuntime || !trafiraRuntimeReady
          ? 'error'
          : runtimeUnstable
            ? 'warning'
            : 'success',
      key: hasByedpiRules
        ? runtimeUnstable
          ? _('Trafira-managed ciadpi runtime has restarted')
          : trafiraRuntimeReady
            ? _('Trafira-managed ciadpi runtime is ready')
            : _('Trafira-managed ciadpi runtime is not ready')
        : unexpectedRuntime
          ? _('Unexpected Trafira-managed ciadpi runtime is running')
          : _('Trafira-managed ciadpi runtime is not running'),
    ...
    },
    {
      ...
      key: standaloneServiceRunning
        ? hasByedpiRules
          ? _('Standalone ByeDPI is active together with Trafira ByeDPI rules')
