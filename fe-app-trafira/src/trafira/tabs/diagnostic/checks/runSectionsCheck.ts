    ...
    async function getLatency(): Promise<{
      ...
      if (section.withTagSelect) {
        ...
        if (selectedOutbound?.code) {
          const latencyProxy =
            await TrafiraShellMethods.getClashApiProxyLatency(
        ...
        const latencyGroup = await TrafiraShellMethods.getClashApiGroupLatency(
          section.code,
        );
      ...
      const latencyProxy = await TrafiraShellMethods.getClashApiProxyLatency(
        section.code,
        section.latencyTestTimeout,
      );
