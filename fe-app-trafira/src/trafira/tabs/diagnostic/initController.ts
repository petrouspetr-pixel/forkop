...
import { TRAFIRA_LUCI_APP_VERSION } from '../../../constants';
...
function isMutatingServiceActionLoading() {
  return (
    ...
    isServiceTransitionStatus(store.get().servicesInfoWidget.data.trafiraStatus)
  );
}

function getTrafiraStatusText(running: boolean, enabled: boolean) {
  if (running) {
...
function setDisplayedTrafiraRunning(running: boolean) {
  const servicesInfoWidget = store.get().servicesInfoWidget;
  const enabled = Boolean(servicesInfoWidget.data.trafiraEnabled);
  ...
  store.set({
    servicesInfoWidget: {
      ...
      data: {
        ...
        trafiraRunning: running ? 1 : 0,
        trafiraStatus: getTrafiraStatusText(running, enabled),
      },
    },
  });
}
...
async function waitForTrafiraRunningState(expectedRunning: boolean) {
  ...
  while (Date.now() - startedAt < SERVICE_ACTION_STATUS_TIMEOUT_MS) {
    ...
    const trafiraRunning = Boolean(
      store.get().servicesInfoWidget.data.trafiraRunning,
    );

    if (trafiraRunning === expectedRunning) {
      return true;
    }
...
async function followServiceActionState(state: Trafira.ServiceActionState) {
  ...
  try {
    if (state.running) {
      await TrafiraShellMethods.waitServiceActionJob(jobId);
    }
  } catch (error) {
    ...
    await refreshDiagnosticServicesInfo({ force: true, allowInactive: true });
    void TrafiraShellMethods.uiActionAck('service', jobId);
    followedServiceActionJobs.delete(jobId);
    resetDiagnosticsChecks();
  }
}

function followServiceActionsFromUiState(uiState?: Trafira.UiState) {
  if (!uiState) {
    return;
  }
...
async function fetchDiagnosticsProviderInfo({
  ...
  try {
    ...
    const [zapretRuntime, zapret2Runtime, byedpiRuntime, inboundsConfig] =
      await Promise.all([
        TrafiraShellMethods.checkZapretRuntime(),
        TrafiraShellMethods.checkZapret2Runtime(),
        TrafiraShellMethods.checkByedpiRuntime(),
        TrafiraShellMethods.checkInboundsConfig(),
      ]);
...
function renderDiagnosticRunActionWidget() {
  ...
  const servicesInfoWidget = store.get().servicesInfoWidget;
  const trafiraRunning = Boolean(servicesInfoWidget.data.trafiraRunning);
...
async function handleServiceRuntimeAction({
  ...
  if (optimisticRunning !== undefined) {
    setDisplayedTrafiraRunning(optimisticRunning);
  }

  try {
    const startResponse = await TrafiraShellMethods.serviceActionStart(action);
    ...
    followedServiceActionJobs.add(jobId);
    ownsJobFollow = true;
    const result = await TrafiraShellMethods.waitServiceActionJob(jobId);
    ...
    await waitForTrafiraRunningState(expectedRunning);
  } catch (e) {
...
async function handleEnable() {
  ...
  try {
    await TrafiraShellMethods.enable();
  } catch (e) {
...
async function handleDisable() {
  ...
  try {
    await TrafiraShellMethods.disable();
  } catch (e) {
...
async function handleShowGlobalCheck() {
  ...
  try {
    const globalCheck = await TrafiraShellMethods.globalCheck(false);
...
async function handleViewLogs() {
  ...
  try {
    const viewLogs = await TrafiraShellMethods.checkLogs();

    if (viewLogs.success) {
      const getLatestLogs = async () => {
        const latestLogs = await TrafiraShellMethods.checkLogs();
...
async function handleShowSingBoxConfig() {
  ...
  try {
    const showSingBoxConfig =
      await TrafiraShellMethods.showSingBoxConfig(false);
...
function renderDiagnosticAvailableActionsWidget() {
  ...
  const trafiraEnabled = Boolean(servicesInfoWidget.data.trafiraEnabled);
  const trafiraRunning = Boolean(servicesInfoWidget.data.trafiraRunning);
  const serviceTransition = getServiceTransition(
    servicesInfoWidget.data.trafiraStatus,
  );
...
function renderDiagnosticSystemInfoWidget() {
  ...
  const items = [
    {
      key: 'Trafira',
      value: normalizeCompiledVersion(diagnosticsSystemInfo.trafira_version),
    },
    {
      key: 'Luci App',
      value: normalizeCompiledVersion(TRAFIRA_LUCI_APP_VERSION),
    },
    {
      key: 'Sing-box',
