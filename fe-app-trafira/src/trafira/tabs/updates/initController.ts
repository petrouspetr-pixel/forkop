...
import { TRAFIRA_ACTION_PROVIDERS_AVAILABILITY_EVENT } from '../../../constants';
...
let componentUpdateCheckCacheSnapshot: Trafira.ComponentUpdateCheckCache | null =
  null;
let componentUpdateCheckCachePromise: Promise<Trafira.ComponentUpdateCheckCache> | null =
  null;
...
function shouldShowInstallAfterCheck(component: Trafira.ComponentName) {
  const status = getVisibleCheckResult(component)?.status;

  return status === 'outdated' || status === 'dev';
}

function getVisibleCheckResult(component: Trafira.ComponentName) {
  if (
    !shouldExposeCheckResults({
...
function getLatestVersion(component: Trafira.ComponentName) {
...
function getGitHubReleaseUrl(component: Trafira.ComponentName) {
  const checkResult = getVisibleCheckResult(component);
...
function isServiceRuntimeActionLoading() {
  ...
  return (
    ...
    isServiceTransitionStatus(state.servicesInfoWidget.data.trafiraStatus)
  );
}
...
function resetCheckResult(component: Trafira.ComponentName) {
  setCheckResult(component, null, '');
}

function applyCachedCheckResults(results: Trafira.ComponentActionResult[]) {
...
function loadComponentUpdateCheckCache({ force = false } = {}) {
  ...
  const promise = TrafiraShellMethods.componentUpdateCheckCache()
    .then((response) =>
      response.success
        ? response.data
        : ({
            enabled: false,
            results: [],
          } satisfies Trafira.ComponentUpdateCheckCache),
    )
    .then((cache) => {
...
async function ackComponentActionJob(jobId: string) {
  try {
    const response = await TrafiraShellMethods.uiActionAck('component', jobId);
...
function getExpectedLatestVersionForAction(button: ComponentActionButton) {
  if (button.component !== 'trafira' || button.action !== 'install') {
    return undefined;
  }
...
function notifyActionProvidersAvailabilityChanged(
  ...
  window.dispatchEvent(
    new CustomEvent(TRAFIRA_ACTION_PROVIDERS_AVAILABILITY_EVENT, {
      detail: {
...
function reloadPageAfterTrafiraUpdate() {
  window.setTimeout(() => {
    window.location.reload();
  }, 1200);
}

function patchSystemInfoAfterMutation(result: Trafira.ComponentActionResult) {
  const systemInfo = store.get().diagnosticsSystemInfo;
  ...
  if (result.component === 'trafira' && result.action === 'install') {
    nextSystemInfo.trafira_version = version;
  }
...
async function applyCompletedComponentAction({
  ...
  key: UpdatesActionKey;
  result: Trafira.ComponentActionResult;
  notify: boolean;
}) {
...
async function completeComponentActionJob(
  key: UpdatesActionKey,
  jobId: string,
  response: Trafira.MethodResponse<Trafira.ComponentActionResult>,
) {
  if (pageUnloading) {
    setActionLoading(key, false);
    return;
...
async function followComponentActionState(
  state: Trafira.ComponentActionResult,
) {
  ...
  try {
    const response = state.running
      ? await TrafiraShellMethods.waitComponentActionJob(
          jobId,
          state.component,
          state.action,
...
function handleComponentUiState(uiState: Trafira.UiState) {
...
async function handleComponentAction(button: ComponentActionButton) {
  ...
  try {
    const startResponse = await TrafiraShellMethods.componentActionStart(
      button.component,
      button.action,
    );
    ...
    const response = await TrafiraShellMethods.waitComponentActionJob(
      jobId,
      button.component,
      button.action,
...
function getComponentCards(): ComponentCard[] {
  ...
  const trafiraActions = getInstalledUpdateActions(
    'trafira',
    'trafiraCheck',
    'trafiraInstall',
  );
  ...
  return [
    {
      ...
      version: systemInfoLoading
        ? _('Loading...')
        : normalizeCompiledVersion(systemInfo.trafira_version),
      latestVersion: getLatestVersion('trafira'),
      releaseUrl: getGitHubReleaseUrl('trafira'),
      actions: trafiraActions,
    },
    {
      component: 'sing_box',
      column: 0,
...
function applyComponentUpdateCheckCache(
  componentUpdateCheckCache: Trafira.ComponentUpdateCheckCache,
) {
