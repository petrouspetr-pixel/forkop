...
function buildRouteDisplayNames(sections: Trafira.ConfigSection[]) {
  ...
  if (serviceAvailability === 'stopped') {
    container.replaceChildren(
      renderConnectionsTable([], {
        text: _(
          'Trafira service is stopped. Start the service to display connections.',
        ),
      }),
    );
    return;
...
async function closeConnection(connectionId: string) {
  ...
  try {
    const response =
      await TrafiraShellMethods.closeClashApiConnection(connectionId);
...
async function closeAllConnections() {
  ...
  try {
    const response = await TrafiraShellMethods.closeAllClashApiConnections();
...
async function loadRouteDisplayNames() {
  try {
    buildRouteDisplayNames(await CustomTrafiraMethods.getConfigSections());
  } catch (error) {
  ...
  serviceStateUnsubscribe = subscribeRuntimeUiState((uiState) => {
    ...
    setServiceAvailability(
      getServiceAvailability({
        loading: false,
        failed: false,
        running: uiState.service.trafira.running,
      }),
    );
  });
}
