...
import { CustomTrafiraMethods, TrafiraShellMethods } from '../../methods';
import {
  logger,
  markUiActionOwned,
...
function setLatencyFetching(
  ...
  fetching: boolean,
  local = false,
  progress?: Trafira.LatencyActionProgress,
) {
  if (local || !fetching) {
...
async function completeSubscriptionUpdateJob(
  jobId: string,
  sectionName: string,
  response: Trafira.MethodResponse<Trafira.SubscriptionUpdateJobState>,
) {
  if (pageUnloading) {
  ...
  if (jobId && response.success) {
    void TrafiraShellMethods.uiActionAck('subscription', jobId);
  }
...
async function followSubscriptionUpdateState(
  state: Trafira.SubscriptionUpdateJobState,
) {
  ...
  try {
    const response = state.running
      ? await TrafiraShellMethods.waitSubscriptionUpdateJob(jobId)
      : ({
          success: true,
          data: state,
...
async function completeLatencyTestJob(jobId: string, sectionName: string) {
  ...
  if (jobId) {
    void TrafiraShellMethods.uiActionAck('latency', jobId);
  }

  void fetchDashboardSections({ force: true });
}

async function followLatencyTestState(state: Trafira.LatencyActionState) {
  const jobId = state.job_id;
  const sectionName = state.section || '';
  ...
  try {
    if (state.running) {
      await TrafiraShellMethods.waitLatencyTestJob(jobId);
    }
...
function followDashboardActionsFromUiState(uiState: Trafira.UiState) {
  for (const state of uiState.actions.subscription || []) {
    if (state.running || (state.job_id && state.section)) {
      ...
      void TrafiraShellMethods.uiActionAck('subscription', state.job_id);
    }
  }
  ...
  for (const state of uiState.actions.latency || []) {
    if (state.running || (state.job_id && state.section)) {
      ...
      void TrafiraShellMethods.uiActionAck('latency', state.job_id);
    }
  }
}
...
function getDashboardServiceAvailability() {
  ...
  return getServiceAvailability({
    ...
    running: service.data.trafiraRunning,
  });
}
...
async function handleChooseOutbound(
  ...
  try {
    await TrafiraShellMethods.setClashApiGroupProxy(selector, tag);
...
function getInitialLatencyProgress(
  ...
  tag: string,
): Trafira.LatencyActionProgress | undefined {
...
async function handleTestLatency(
  ...
  try {
    const startResponse = await TrafiraShellMethods.latencyTestStart(
      latencyType,
      sectionName,
      tag,
      timeout,
...
function handleCopyOutbound(outbound: Trafira.Outbound) {
...
function renderDetailsMemberName(member: Trafira.UrlTestMember) {
...
function renderUrlTestSelectedValue(info: Trafira.UrlTestInfo) {
...
function renderUrlTestInfoModal(outbound: Trafira.Outbound) {
...
function handleShowUrlTestInfo(outbound: Trafira.Outbound) {
...
function renderPrioritySelectedValue(info: Trafira.PriorityInfo) {
  const selectedMember = info.outbounds.find((member) => member.selected);
  const selectedName =
    selectedMember?.displayName || info.selectedName || info.selectedCode || '';
  const name = formatUrlTestModalValue(selectedName);
...
function renderPriorityMemberName(member: Trafira.PriorityMember) {
...
function renderPriorityInfoModal(outbound: Trafira.Outbound) {
...
function handleShowPriorityInfo(outbound: Trafira.Outbound) {
...
async function handleUpdateSubscription(section: Trafira.OutboundGroup) {
  if (
  ...
  try {
    const startResponse = await TrafiraShellMethods.subscriptionUpdateStart(
      section.sectionName,
    );
    ...
    followedSubscriptionJobs.add(jobId);
    ownsJobFollow = true;
    const response = await TrafiraShellMethods.waitSubscriptionUpdateJob(jobId);
...
async function renderServicesInfoWidget() {
  ...
  const renderedWidget = renderWidget({
    ...
    items: [
      {
        key: 'Trafira',
        value: servicesInfoWidget.data.trafiraRunning
          ? _('✔ Running')
          : _('✘ Stopped'),
        attributes: {
          class: servicesInfoWidget.data.trafiraRunning
