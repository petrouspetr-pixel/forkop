...
const targetRoot = path.resolve(__dirname, '../luci-app-trafira/po');
...
async function main() {
    ...
    for (const file of files) {
        ...
        if (file === 'trafira.pot') {
            const potTarget = path.join(targetRoot, 'templates', 'trafira.pot');
        ...
        const match = file.match(/^trafira\.([a-zA-Z_]+)\.po$/);
        if (match) {
            const lang = match[1];
            const poTarget = path.join(targetRoot, lang, 'trafira.po');
