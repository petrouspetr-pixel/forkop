...
const SERVER_STYLE_ID = "trafira-server-styles";
...
function defaultTlsCertificatePath(sectionId) {
  return `/etc/trafira/server-certs/${serverSafeName(sectionId)}.crt`;
}

function defaultTlsKeyPath(sectionId) {
  return `/etc/trafira/server-certs/${serverSafeName(sectionId)}.key`;
}

function defaultTailscaleHostname(sectionId) {
  return `trafira-${serverSafeName(sectionId)}`;
}
...
function generateRealityKeypair(sectionId) {
  return fs
    .exec("/usr/bin/trafira", ["generate_reality_keypair"])
    .then((response) => {
      if ((response.code ?? 0) !== 0 || !response.stdout) {
        throw new Error(response.stderr || response.stdout || "");
      }
