...
const UCI_PACKAGE = main.TRAFIRA_UCI_PACKAGE;
const ACTION_PROVIDERS_AVAILABILITY_EVENT =
  main.TRAFIRA_ACTION_PROVIDERS_AVAILABILITY_EVENT ||
  "trafira:action-providers-availability";
...
const SECTION_CACHE_DIR = "/var/run/trafira/section-cache";
const COUNTRY_CODES =
...
function ensureActionProvidersAvailabilityLoaded() {
  ...
  actionProvidersAvailabilityPromise = Promise.allSettled([
    main.TrafiraShellMethods.checkZapretRuntime(),
    main.TrafiraShellMethods.checkZapret2Runtime(),
    main.TrafiraShellMethods.checkByedpiRuntime(),
  ])
...
function attachAnnotatedTextarea(textarea, analyzer) {
  ...
  if (textarea.__trafiraAnnotatedTextareaController) {
    textarea.__trafiraAnnotatedTextareaController.analyzer = analyzer;
    textarea.__trafiraAnnotatedTextareaController.update();
    return;
  }
  ...
  textarea.__trafiraAnnotatedTextareaController = controller;
...
function refreshAnnotatedTextareaValidation(option, section_id, textarea) {
  ...
  if (
    textarea &&
    textarea.__trafiraAnnotatedTextareaController &&
    typeof textarea.__trafiraAnnotatedTextareaController.update === "function"
  ) {
    textarea.__trafiraAnnotatedTextareaController.update();
  }
}
...
function attachNfqwsRemoteValidation(option, section_id, textarea) {
  if (!textarea || textarea.__trafiraNfqwsRemoteValidationAttached) {
    return;
  }

  textarea.__trafiraNfqwsRemoteValidationAttached = true;
  textarea.__trafiraNfqwsRemoteValidationRequestId = 0;
  textarea.__trafiraNfqwsRemoteValidationTimer = null;
  ...
  const runValidation = () => {
    ...
    const requestId =
      (textarea.__trafiraNfqwsRemoteValidationRequestId || 0) + 1;
    textarea.__trafiraNfqwsRemoteValidationRequestId = requestId;

    validateNfqwsStrategyRemotely(value).then(() => {
      if (textarea.__trafiraNfqwsRemoteValidationRequestId !== requestId) {
        return;
      }
  ...
  const scheduleValidation = (delay = NFQWS_REMOTE_VALIDATION_DEBOUNCE_MS) => {
    if (textarea.__trafiraNfqwsRemoteValidationTimer) {
      window.clearTimeout(textarea.__trafiraNfqwsRemoteValidationTimer);
    }

    textarea.__trafiraNfqwsRemoteValidationTimer = window.setTimeout(() => {
      textarea.__trafiraNfqwsRemoteValidationTimer = null;
      runValidation();
    }, delay);
  };
...
function attachNfqws2RemoteValidation(option, section_id, textarea) {
  if (!textarea || textarea.__trafiraNfqws2RemoteValidationAttached) {
    return;
  }

  textarea.__trafiraNfqws2RemoteValidationAttached = true;
  textarea.__trafiraNfqws2RemoteValidationRequestId = 0;
  textarea.__trafiraNfqws2RemoteValidationTimer = null;
  ...
  const runValidation = () => {
    ...
    const requestId =
      (textarea.__trafiraNfqws2RemoteValidationRequestId || 0) + 1;
    textarea.__trafiraNfqws2RemoteValidationRequestId = requestId;

    validateNfqws2StrategyRemotely(value).then(() => {
      if (textarea.__trafiraNfqws2RemoteValidationRequestId !== requestId) {
        return;
      }
  ...
  const scheduleValidation = (delay = NFQWS_REMOTE_VALIDATION_DEBOUNCE_MS) => {
    if (textarea.__trafiraNfqws2RemoteValidationTimer) {
      window.clearTimeout(textarea.__trafiraNfqws2RemoteValidationTimer);
    }

    textarea.__trafiraNfqws2RemoteValidationTimer = window.setTimeout(() => {
      textarea.__trafiraNfqws2RemoteValidationTimer = null;
      runValidation();
    }, delay);
  };
...
function getNfqwsForbiddenTokenInfo(token, index) {
  const configFileMessage = _(
    "External nfqws config files bypass Trafira queue management and explicit validation.",
  );
  ...
  const placeholderMessage = _(
    "Zapret hostlist templates are not supported here because Trafira does not expand them for per-rule NFQWS strategies.",
  );
  const queueMessage = _(
    "The NFQUEUE number is assigned by Trafira for each rule and must not be overridden here.",
  );
  const fwmarkMessage = _(
    "The desync fwmark is managed by Trafira for loop prevention and must not be overridden here.",
  );
  const daemonMessage = _(
    "Trafira manages the nfqws process lifecycle itself, so daemon mode is not allowed here.",
  );
...
function getNfqws2ForbiddenTokenInfo(token, index, nextToken) {
  const configFileMessage = _(
    "External nfqws2 config files bypass Trafira queue management and explicit validation.",
  );
  const hostSelectionMessage = _(
  ...
  const placeholderMessage = _(
    "Zapret2 hostlist templates are not supported here because Trafira does not expand them for per-rule NFQWS2 strategies.",
  );
  const queueMessage = _(
    "The NFQUEUE number is assigned by Trafira for each rule and must not be overridden here.",
  );
  ...
  const fuzzMessage = _(
    "Fuzzing is not supported here because Trafira needs deterministic runtime validation.",
  );
  const interceptMessage = _(
    "Disabling interception is incompatible with action=zapret2 because Trafira sends matched traffic through NFQUEUE.",
  );
  const daemonMessage = _(
    "Trafira manages the nfqws2 process lifecycle itself, so daemon mode is not allowed here.",
  );
  const dryRunMessage = _(
...
function getByedpiControlledTokenInfo(token) {
  const listenMessage = _(
    "ByeDPI listen address and port are assigned by Trafira and must not be set in the strategy.",
  );
  const transparentMessage = _(
    "Transparent proxy mode is incompatible with action=byedpi because Trafira connects to ciadpi through SOCKS.",
  );
  const daemonMessage = _(
    "Trafira manages the ciadpi process lifecycle itself, so daemon mode is not allowed here.",
  );
  const pidfileMessage = _(
    "Trafira manages ciadpi pid files itself, so pidfile options are not allowed here.",
  );
  const exitMessage = _(
...
function createSectionContent(section) {
  ...
  o = section.taboption(
    ...
    "action",
    _("Action"),
    _("What Trafira should do when this section matches"),
  );
