...
// src/trafira/services/logger.service.ts
var Logger = class {
  constructor() {
    this.logs = [];
...
// src/constants.ts
var TRAFIRA_UCI_PACKAGE = "trafira";
var TRAFIRA_LUCI_APP_VERSION = "__COMPILED_VERSION_VARIABLE__";
var TRAFIRA_ACTION_PROVIDERS_AVAILABILITY_EVENT = "trafira:action-providers-availability";
...
// src/trafira/tabs/dashboard/partials/renderSections.ts
...
// src/trafira/tabs/dashboard/partials/renderWidget.ts
...
// src/trafira/tabs/dashboard/render.ts
function render() {
  return E(
    "div",
    {
...
// src/trafira/methods/custom/getConfigSections.ts
async function getConfigSections() {
  return uci.load(TRAFIRA_UCI_PACKAGE).then(() => uci.sections(TRAFIRA_UCI_PACKAGE));
}

// src/trafira/runtimeTags.ts
...
((Trafira2) => {
  ...
  })(AvailableMethods = Trafira2.AvailableMethods || (Trafira2.AvailableMethods = {}));
  ...
  })(AvailableClashAPIMethods = Trafira2.AvailableClashAPIMethods || (Trafira2.AvailableClashAPIMethods = {}));
})(Trafira || (Trafira = {}));

// src/trafira/helpers/isTransientRpcError.ts
...
// src/trafira/methods/shell/index.ts
...
var COMPONENT_ACTION_STATE_DIR = "/var/run/trafira/component-actions";
...
var TrafiraShellMethods = {
  ...
  getStatus: async () => callBaseMethod(Trafira.AvailableMethods.GET_STATUS),
  getOutboundMetadata: async (section) => callBaseMethod(
    Trafira.AvailableMethods.GET_OUTBOUND_METADATA,
    [section]
  ),
  getSubscriptionMetadata: async (section) => callBaseMethod(Trafira.AvailableMethods.GET_SUBSCRIPTION_METADATA, [section]),
  ...
  getClashApiProxies: async () => callBaseMethod(Trafira.AvailableMethods.CLASH_API, [
    Trafira.AvailableClashAPIMethods.GET_PROXIES
  ]),
  getClashApiConnections: async () => callBaseMethod(Trafira.AvailableMethods.CLASH_API, [
  ...
  setClashApiGroupProxy: async (group, proxy) => callBaseMethod(Trafira.AvailableMethods.CLASH_API, [
    Trafira.AvailableClashAPIMethods.SET_GROUP_PROXY,
    group,
    proxy
  ]),
  closeClashApiConnection: async (connectionId) => callBaseMethod(Trafira.AvailableMethods.CLASH_API, [
    Trafira.AvailableClashAPIMethods.CLOSE_CONNECTION,
    connectionId
  ]),
  closeAllClashApiConnections: async () => callBaseMethod(Trafira.AvailableMethods.CLASH_API, [
  ...
  globalCheck: async (masked = true) => callBaseMethod(Trafira.AvailableMethods.GLOBAL_CHECK, [
    masked ? "masked" : "raw"
  ]),
  showSingBoxConfig: async (masked = true) => callBaseMethod(Trafira.AvailableMethods.SHOW_SING_BOX_CONFIG, [
    masked ? "masked" : "raw"
  ]),
  checkLogs: async () => callBaseMethod(Trafira.AvailableMethods.CHECK_LOGS),
    ...
    while (true) {
      ...
      if ((statusResponse.code ?? 0) !== 0 || !parsedResponse) {
        ...
        if (component === "trafira" && action === "install") {
          const installedVersion = expectedLatestVersion ? await readTrafiraVersion() : "";
  ...
  waitSubscriptionUpdateJob: async (jobId) => {
    ...
    while (true) {
      ...
      const response = await TrafiraShellMethods.subscriptionUpdateStatus(jobId);
      if (!response.success) {
        if (transientRpc.shouldContinue(response.error)) {
          continue;
        }
...
// src/trafira/methods/custom/getDashboardSections.ts
var DASHBOARD_SECTION_CACHE_DIR = "/var/run/trafira/section-cache";
...
// src/trafira/methods/custom/getClashApiSecret.ts
...
// src/trafira/methods/custom/index.ts
var CustomTrafiraMethods = {
  getConfigSections,
...
// src/trafira/methods/fakeip/getFakeIpCheck.ts
...
// src/trafira/methods/fakeip/getIpCheck.ts
...
// src/trafira/methods/fakeip/index.ts
var RemoteFakeIPMethods = {
  getFakeIpCheck,
  getIpCheck
};

// src/trafira/services/tab.service.ts
...
// src/trafira/tabs/diagnostic/helpers/getCheckTitle.ts
function getCheckTitle(name) {
  return `${name} ${_("checks")}`;
}

// src/trafira/tabs/diagnostic/checks/contstants.ts
...
// src/trafira/tabs/diagnostic/diagnostic.store.ts
...
// src/trafira/services/store.service.ts
...
// src/trafira/services/trafiraLogWatcher.service.ts
var TrafiraLogWatcher = class _TrafiraLogWatcher {
  constructor() {
    this.intervalMs = 5e3;
  ...
  static getInstance() {
    if (!_TrafiraLogWatcher.instance) {
      _TrafiraLogWatcher.instance = new _TrafiraLogWatcher();
    }
    return _TrafiraLogWatcher.instance;
  }
...
// src/trafira/services/logNotificationDeduper.service.ts
var LOG_NOTIFICATION_STORAGE_KEY = "trafira:shown-log-error-notifications:v1";
...
function getTrafiraLogNotification(line) {
...
// src/trafira/helpers/getComponentActionKey.ts
...
// src/trafira/helpers/singBoxVariant.ts
...
// src/trafira/services/localActionOverlay.service.ts
...
// src/trafira/services/uiState.service.ts
...
// src/trafira/services/runtimeUiState.service.ts
...
async function refreshRuntimeUiState({
  ...
  const promise = TrafiraShellMethods.getUiState().then((response) => {
    if (!response.success) {
      return void 0;
    }
...
// src/trafira/services/core.service.ts
...
// src/trafira/services/socket.service.ts
...
// src/trafira/services/uiActionNotification.service.ts
var UI_ACTION_NOTIFICATION_STORAGE_KEY = "trafira:owned-ui-action-notifications:v1";
...
// src/trafira/fetchers/fetchServicesInfo.ts
  ...
  store.set({
    servicesInfoWidget: {
      ...
      data: {
        ...
        trafiraRunning: trafira.success ? trafira.data.running : previousData.trafiraRunning,
        trafiraEnabled: trafira.success ? trafira.data.enabled : previousData.trafiraEnabled,
        trafiraStatus: trafira.success ? trafira.data.status : previousData.trafiraStatus
      }
    }
  });
  return void 0;
...
// src/trafira/helpers/isActiveLuciTab.ts
...
// src/trafira/helpers/restoredActionLoading.ts
function shouldShowLoadingForRestoredAction(state) {
  return state.running === true;
}

// src/trafira/helpers/serviceAvailability.ts
...
async function followSubscriptionUpdateState(state) {
  ...
  try {
    const response = state.running ? await TrafiraShellMethods.waitSubscriptionUpdateJob(jobId) : {
      success: true,
      data: state
    };
...
async function renderServicesInfoWidget() {
  ...
  const renderedWidget = renderWidget({
    ...
    items: [
      {
        key: "Trafira",
        value: servicesInfoWidget.data.trafiraRunning ? _("\u2714 Running") : _("\u2718 Stopped"),
        attributes: {
          class: servicesInfoWidget.data.trafiraRunning ? "fkp_dashboard-page__widgets-section__item__row--success" : "fkp_dashboard-page__widgets-section__item__row--error"
        }
      },
      {
        key: "Sing-box",
...
// src/trafira/tabs/dashboard/styles.ts
var styles = `
@font-face {
...
#cbi-${TRAFIRA_UCI_PACKAGE}-dashboard-_mount_node > .cbi-value-title {
    display: none;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-dashboard-_mount_node > .cbi-value-field {
    margin-left: 0;
    width: 100%;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-dashboard-_mount_node > div {
    width: 100%;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-dashboard > h3 {
    display: none;
}
...
// src/trafira/tabs/dashboard/index.ts
var DashboardTab = {
  render,
  initController,
  styles
...
// src/trafira/tabs/diagnostic/renderDiagnostic.ts
function render2() {
...
// src/trafira/tabs/diagnostic/helpers/getMeta.ts
...
// src/trafira/tabs/diagnostic/checks/getDnsCheckPresentation.ts
...
// src/trafira/tabs/diagnostic/checks/runDnsCheck.ts
...
// src/trafira/tabs/diagnostic/checks/runSingBoxCheck.ts
...
// src/trafira/tabs/diagnostic/checks/runInboundsCheck.ts
...
// src/trafira/tabs/diagnostic/checks/runNftCheck.ts
...
// src/trafira/tabs/diagnostic/checks/runFakeIPCheck.ts
...
// src/trafira/tabs/diagnostic/checks/getCheckItemsMeta.ts
...
// src/trafira/tabs/diagnostic/checks/runZapretCheck.ts
async function runZapretCheck() {
  ...
  const trafiraRuntimeReady = !hasZapretRules || runningProcesses === expectedProcesses && supervisorProcesses === expectedProcesses;
  ...
  const items = [
    ...
    },
    {
      ...
      key: hasZapretRules ? trafiraRuntimeReady ? _("Trafira-managed nfqws runtime is ready") : _("Trafira-managed nfqws runtime is not ready") : unexpectedRuntime ? _("Unexpected Trafira-managed nfqws runtime is running") : _("Trafira-managed nfqws runtime is not running"),
    ...
    },
    {
      ...
      key: standaloneServiceRunning ? hasZapretRules ? _("Standalone Zapret is active together with Trafira Zapret rules") : _("Standalone Zapret service is active") : _("Standalone Zapret service is inactive"),
      value: ""
    }
  ];
...
// src/trafira/tabs/diagnostic/checks/runZapret2Check.ts
async function runZapret2Check() {
  ...
  const runningProcesses = Number(data.running_process_count || 0);
  const supervisorProcesses = Number(data.supervisor_process_count || 0);
  const trafiraRuntimeReady = !hasZapret2Rules || runningProcesses === expectedProcesses && supervisorProcesses === expectedProcesses;
  ...
  const items = [
    ...
    {
      state: unexpectedRuntime || !trafiraRuntimeReady ? "error" : "success",
      key: hasZapret2Rules ? trafiraRuntimeReady ? _("Trafira-managed nfqws2 runtime is ready") : _("Trafira-managed nfqws2 runtime is not ready") : unexpectedRuntime ? _("Unexpected Trafira-managed nfqws2 runtime is running") : _("Trafira-managed nfqws2 runtime is not running"),
...
// src/trafira/tabs/diagnostic/checks/runByedpiCheck.ts
async function runByedpiCheck() {
  ...
  const trafiraRuntimeReady = !hasByedpiRules || runningProcesses === expectedProcesses && supervisorProcesses === expectedProcesses;
  ...
  const items = [
    ...
    {
      state: unexpectedRuntime || !trafiraRuntimeReady ? "error" : runtimeUnstable ? "warning" : "success",
      key: hasByedpiRules ? runtimeUnstable ? _("Trafira-managed ciadpi runtime has restarted") : trafiraRuntimeReady ? _("Trafira-managed ciadpi runtime is ready") : _("Trafira-managed ciadpi runtime is not ready") : unexpectedRuntime ? _("Unexpected Trafira-managed ciadpi runtime is running") : _("Trafira-managed ciadpi runtime is not running"),
    ...
    {
      state: standaloneConflict ? "error" : standaloneAutostartRisk ? "warning" : "success",
      key: standaloneServiceRunning ? hasByedpiRules ? _("Standalone ByeDPI is active together with Trafira ByeDPI rules") : _("Standalone ByeDPI service is active") : standaloneAutostartRisk ? _("Standalone ByeDPI autostart is enabled") : _("Standalone ByeDPI service is inactive"),
      value: ""
    }
  ];
  const { state, description } = getCheckItemsMeta(items);
...
// src/trafira/services/systemInfo.service.ts
...
// src/trafira/tabs/diagnostic/partials/renderAvailableActions.ts
...
// src/trafira/tabs/diagnostic/partials/renderCheckSection.ts
...
// src/trafira/tabs/diagnostic/partials/renderRunAction.ts
...
// src/trafira/tabs/diagnostic/partials/renderSystemInfo.ts
...
// src/trafira/tabs/diagnostic/partials/renderWikiDisclaimer.ts
...
// src/trafira/tabs/diagnostic/checks/runSectionsCheck.ts
    ...
    async function getLatency() {
      if (section.withTagSelect) {
        ...
        if (selectedOutbound2?.code) {
          const latencyProxy2 = await TrafiraShellMethods.getClashApiProxyLatency(
...
function shouldDisableDiagnosticRunAction({
  ...
  return !providerInfoLoaded || servicesInfoLoading || !trafiraRunning || mutatingServiceActionLoading;
}
...
// src/trafira/tabs/diagnostic/diagnosticRunPersistence.ts
var DIAGNOSTIC_RUN_STORAGE_KEY = "trafira:diagnostic-run:v1";
...
// src/trafira/tabs/diagnostic/helpers/maskDiagnostics.ts
...
var TRAFIRA_MASK_AFTER_TOKEN_SPACE = [
...
// src/trafira/tabs/diagnostic/initController.ts
...
function isMutatingServiceActionLoading() {
  return isLocalMutatingServiceActionLoading() || isServiceTransitionStatus(store.get().servicesInfoWidget.data.trafiraStatus);
}
function getTrafiraStatusText(running, enabled) {
  if (running) {
    return enabled ? "running & enabled" : "running but disabled";
  }
  return enabled ? "stopped but enabled" : "stopped & disabled";
}
function setDisplayedTrafiraRunning(running) {
  const servicesInfoWidget = store.get().servicesInfoWidget;
...
async function waitForTrafiraRunningState(expectedRunning) {
...
async function handleShowSingBoxConfig() {
  ...
  try {
    const showSingBoxConfig = await TrafiraShellMethods.showSingBoxConfig(false);
...
// src/trafira/tabs/diagnostic/styles.ts
var styles4 = `

#cbi-${TRAFIRA_UCI_PACKAGE}-diagnostic-_mount_node > div {
    width: 100%;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-diagnostic > h3 {
    display: none;
}
...
// src/trafira/tabs/monitoring/initController.ts
...
async function closeConnection(connectionId) {
  ...
  renderConnections();
  try {
    const response = await TrafiraShellMethods.closeClashApiConnection(connectionId);
    if (!response.success) {
...
// src/trafira/tabs/monitoring/styles.ts
var styles5 = `
#cbi-${TRAFIRA_UCI_PACKAGE}-monitoring-_mount_node {
    margin: 16px 0 22px;
    padding: 0;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-monitoring-_mount_node > .cbi-value-title {
    display: none;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-monitoring-_mount_node > .cbi-value-field {
    margin-left: 0;
    width: 100%;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-monitoring-_mount_node > div {
    width: 100%;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-monitoring > h3 {
    display: none;
}
...
// src/trafira/tabs/updates/componentActionCompletion.ts
...
function isServiceRuntimeActionLoading() {
  ...
  return hasLocalMutatingServiceActionLoading(state.diagnosticsActions) || isServiceTransitionStatus(state.servicesInfoWidget.data.trafiraStatus);
}
...
function loadComponentUpdateCheckCache({ force = false } = {}) {
  ...
  const promise = TrafiraShellMethods.componentUpdateCheckCache().then(
...
async function followComponentActionState(state) {
  ...
  try {
    const response = state.running ? await TrafiraShellMethods.waitComponentActionJob(
      jobId,
      state.component,
      state.action,
  ...
  return [
    {
      ...
      column: 0,
      title: "Trafira",
      version: systemInfoLoading ? _("Loading...") : normalizeCompiledVersion(systemInfo.trafira_version),
...
// src/trafira/tabs/updates/styles.ts
var styles6 = `
#cbi-${TRAFIRA_UCI_PACKAGE}-updates-_mount_node > div {
    width: 100%;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-updates > h3 {
    display: none;
}
...
#cbi-${TRAFIRA_UCI_PACKAGE}-settings > h3 {
    display: none;
}

/* Hide extra H3 for rules tab */
#cbi-${TRAFIRA_UCI_PACKAGE}-section > h3:nth-child(1) {
    display: none;
}

/* Vertical align for remove rule action button */
#cbi-${TRAFIRA_UCI_PACKAGE}-section > .cbi-section-remove {
    margin-bottom: -32px;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-section .cbi-section-actions > div {
    display: inline-flex;
    align-items: center;
    gap: 4px;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-section .cbi-section-actions {
    text-align: right;
}

/* Rule reorder visuals */
#cbi-${TRAFIRA_UCI_PACKAGE}-section {
    position: relative;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-section .cbi-section-table-row {
    position: relative;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-section .cbi-section-table-row.placeholder {
    opacity: 1;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-section .cbi-section-table-row.placeholder em {
    font-style: italic;
}

#cbi-${TRAFIRA_UCI_PACKAGE}-section .cbi-section-table-row.drag-over-above::after,
#cbi-${TRAFIRA_UCI_PACKAGE}-section .cbi-section-table-row.drag-over-below::after {
    content: '';
    position: absolute;
    left: 10px;
    right: 10px;
...
var TRAFIRA_GLOBAL_STYLES_ID = "trafira-global-styles";
