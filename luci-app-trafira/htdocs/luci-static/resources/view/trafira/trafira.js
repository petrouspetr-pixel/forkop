...
// Server
"require view.trafira.server as server";

// Dashboard
"require view.trafira.dashboard as dashboard";

// Monitoring
"require view.trafira.monitoring as monitoring";

// Diagnostic
"require view.trafira.diagnostic as diagnostic";

// Updates
"require view.trafira.updates as updates";
  ...
  async render() {
    ...
    const applyUiCapabilities = function () {
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(
          new CustomEvent(main.TRAFIRA_ACTION_PROVIDERS_AVAILABILITY_EVENT, {
            detail: {
        ...
        main.store.set({
          servicesInfoWidget: {
            ...
            data: {
              ...
              trafiraRunning: Number(data.service.trafira?.running) || 0,
              trafiraEnabled: Number(data.service.trafira?.enabled) || 0,
              trafiraStatus: data.service.trafira?.status || "",
            },
          },
        });
      }
    ...
    const loadFallbackUiCapabilities = function () {
      return Promise.allSettled([
        main.TrafiraShellMethods.getServerCapabilities(),
        ...
        main.TrafiraShellMethods.checkInboundsConfig(),
      ]).then(
        ([
    ...
    const loadUiCapabilities = function () {
      ...
      uiCapabilitiesPromise = main.TrafiraShellMethods.getUiCapabilities()
        .then((response) => {
          if (!response?.success) {
        ...
        .catch((error) => {
          console.warn("Failed to load Trafira UI capabilities", error);
          return main.TrafiraShellMethods.getUiState()
            .then((response) => {
              if (!response?.success) {
            ...
            .catch((fallbackError) => {
              console.warn("Failed to load Trafira UI state", fallbackError);
    ...
    const trafiraMap = new form.Map(
      UCI_PACKAGE,
      _("Trafira Settings"),
      _("Configuration for Trafira service"),
    );
    trafiraMap.tabbed = true;
    const originalHandleSaveApply = trafiraMap.handleSaveApply;
    trafiraMap.handleSaveApply = function (ev, mode) {
      const refreshUiState = function () {
        main.TrafiraShellMethods.getUiState()
          .then((response) => {
            if (
              response?.success &&
    ...
    const rulesSection = trafiraMap.section(
      form.GridSection,
      "section",
      _("Sections"),
    ...
    const serverSection = trafiraMap.section(
      form.GridSection,
      "server",
      _("Servers"),
    ...
    const settingsSection = trafiraMap.section(
      form.TypedSection,
      "settings",
      _("Settings"),
    );
    ...
    const dashboardSection = trafiraMap.section(
      form.TypedSection,
      "dashboard",
      _("Dashboard"),
    );
    ...
    const monitoringSection = trafiraMap.section(
      form.TypedSection,
      "monitoring",
      _("Monitoring"),
    );
    ...
    const updatesSection = trafiraMap.section(
      form.TypedSection,
      "updates",
      _("Components"),
    );
    ...
    const rendered = await trafiraMap.render();
    main.coreService({
